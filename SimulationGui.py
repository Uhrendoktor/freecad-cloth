...
The module keeps FreeCAD/Qt imports lazy so importing the command module remains
safe in headless test environments.  The task panel edits the persistent
...
class SimulationTaskPanel:
    ...
    MATERIALS = {
        "Cotton": (0.35, 0.20, 0.01),
        "Silk": (0.12, 0.08, 0.005),
        "Denim": (0.65, 0.45, 0.02),
        "Wool": (0.45, 0.30, 0.015),
    }
    ...
    def __init__(self, scene=None):
        ...
        selection_layout = QtWidgets.QFormLayout(selection)
        ...
        selection_layout.addRow("Collision object", self.avatar)
        root.addWidget(selection)

        material = QtWidgets.QGroupBox("Fabric")
        material_layout = QtWidgets.QFormLayout(material)
        self.material = QtWidgets.QComboBox()
        self.material.addItems(tuple(self.MATERIALS))
        ...
        material_layout.addRow("Preset", self.material)
        material_layout.addRow("Stretch compliance", self.stretch)
        material_layout.addRow("Bend compliance", self.bend)
        material_layout.addRow("Areal density", self.density)
        root.addWidget(material)

        solver = QtWidgets.QGroupBox("Solver")
        solver_layout = QtWidgets.QFormLayout(solver)
        ...
        solver_layout.addRow("Iterations", self.iterations)
        solver_layout.addRow("Time step", self.timestep)
        ...
        solver_layout.addRow("Steps", self.steps)
        root.addWidget(solver)

        collision = QtWidgets.QGroupBox("Collision")
        collision_layout = QtWidgets.QFormLayout(collision)
        ...
        collision_layout.addRow("Thickness", self.thickness)
        collision_layout.addRow("Mesh deflection", self.deflection)
        collision_layout.addRow("Sphere radius", self.collision_radius)
        root.addWidget(collision)

        sewing = QtWidgets.QGroupBox("Sewing / pinning")
        sewing_layout = QtWidgets.QFormLayout(sewing)
        ...
        sewing_layout.addRow("Pinned vertices", self.pins)
        sewing_layout.addRow("Seam pairs", self.seams)
        ...
        for widget in (self.iterations, self.timestep, self.gravity_x, self.gravity_y,
                       self.gravity_z, self.collision_radius):
            widget.valueChanged.connect(self._parameters_changed)
        self.thickness.valueChanged.connect(self._collision_changed)
        self.deflection.valueChanged.connect(self._collision_changed)
    ...
    def _ensure_property(self, name, type_name, group, default):
        if self.scene is None or hasattr(self.scene, name):
            return
        self.scene.addProperty(type_name, name, group)
        setattr(self.scene, name, default)

    def _load_scene_values(self):
        if self.scene is None:
            return
        self._ensure_property("MaterialPreset", "App::PropertyString", "Fabric", "Cotton")
        self._ensure_property("StretchCompliance", "App::PropertyFloat", "Fabric", 0.35)
        self._ensure_property("BendCompliance", "App::PropertyFloat", "Fabric", 0.20)
        self._ensure_property("ArealDensity", "App::PropertyFloat", "Fabric", 0.01)
        preset = str(getattr(self.scene, "MaterialPreset", "Cotton"))
        index = self.material.findText(preset)
    ...
    def _material_changed(self, preset):
        ...
        self.stretch.setValue(values[0])
        self.bend.setValue(values[1])
        self.density.setValue(values[2])
        self._ensure_property("MaterialPreset", "App::PropertyString", "Fabric", "Cotton")
        self._ensure_property("StretchCompliance", "App::PropertyFloat", "Fabric", 0.35)
        self._ensure_property("BendCompliance", "App::PropertyFloat", "Fabric", 0.20)
        self._ensure_property("ArealDensity", "App::PropertyFloat", "Fabric", 0.01)
        self.scene.MaterialPreset = str(preset)
        self.scene.StretchCompliance = values[0]
        self.scene.BendCompliance = values[1]
        self.scene.ArealDensity = values[2]
        self.scene.Document.recompute()
    ...
    def _parameters_changed(self):
        ...
        self.scene.GravityX = self.gravity_x.value()
        self.scene.GravityY = self.gravity_y.value()
        self.scene.GravityZ = self.gravity_z.value()
        self.scene.CollisionRadius = self.collision_radius.value()
        self.scene.Document.recompute()
        self._refresh_status()
    ...
    def _collision_changed(self):
        ...
        avatar = getattr(self.scene, "AvatarProxy", None)
        if avatar is not None:
            avatar.CollisionThickness = self.thickness.value()
            avatar.CollisionDeflection = self.deflection.value()
        self.scene.Document.recompute()
        self._refresh_status()
    ...
    def reset(self):
        ...
        self._refresh_status("Simulation reset; selections retained.")
    ...
    def _refresh_status(self, message=None):
        if self.scene is None:
            ...
            return
        finite = bool(getattr(self.scene, "FiniteState", True))
        state = "ready" if finite else "invalid/non-finite"
        self.status.setText(message or "State: %s | %.3f s | %d particles | %d steps" % (
            state,
    ...
    def accept(self):
        self._ensure_scene().Document.recompute()
        return True
