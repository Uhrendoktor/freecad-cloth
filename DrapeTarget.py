...
class DrapeTargetSpec:
    target_type: str = "FreeCAD Geometry"; source_name: str = ""; deflection: float = 1.0; thickness: float = 0.0; enabled: bool = True
...
def collision_surface(target,deflection=1.0,thickness=0.0)->CollisionSurface: return surface_from_freecad(target,float(deflection),float(thickness))
...
def _geometry_signature(target):
    ...
    mesh=getattr(target,"Mesh",None); topology=getattr(mesh,"Topology",None) if mesh is not None else None
    if topology is not None:
        try:
            vertices,triangles=topology; return ("Mesh",len(vertices),len(triangles))
...
def _object_signature(obj):
    ...
    ignored={"SourceSignature","DependencySignature","PatternSignature","SewingSignature","AvatarSignature","ArrangementSignature","CollisionParametersSignature","SourcePlacementSignature","TargetStatus","LifecycleState","InvalidationReason"}; props=[]
...
def dependency_signatures(target):
    ...
    return {"pattern geometry":tuple(sorted((_object_signature(o) for o in _links(target,"PatternDependencies")),key=repr)),"sewing topology":tuple(sorted((_object_signature(o) for o in _links(target,"SewingDependencies")),key=repr)),"avatar":tuple(sorted((_object_signature(o) for o in _links(target,"AvatarDependencies")),key=repr)),"arrangement":tuple(sorted((_object_signature(o) for o in _links(target,"ArrangementDependencies")),key=repr)),"collision parameters":(float(getattr(target,"CollisionDeflection",1.0)),float(getattr(target,"CollisionThickness",0.0))),"source placement":(str(getattr(source,"Name","")),_geometry_signature(source),_placement_signature(source))}

def _dependency_digest(values): return hashlib.sha256(repr(tuple((k,values[k]) for k in DEPENDENCY_CLASSES)).encode("utf-8")).hexdigest()

def _changed_reasons(target,current):
    pairs=(("pattern geometry","PatternSignature"),("sewing topology","SewingSignature"),("avatar","AvatarSignature"),("arrangement","ArrangementSignature"),("collision parameters","CollisionParametersSignature"),("source placement","SourcePlacementSignature"))
...
def target_status(target):
    ...
    state=str(getattr(target,"LifecycleState","INVALID")); state=state if state in STATES else "INVALID"
    ...
    if not persisted or vertices<=0 or triangles<=0: return _status(target,"STALE","Drape target collision surface needs to be refreshed","collision cache missing")
    if digest!=str(persisted): return _status(target,"STALE","Drape target is stale; refresh before simulation",", ".join(reasons) if reasons else "dependency changed")
...
def _set_signatures(target,current):
    target.DependencySignature=_dependency_digest(current); target.PatternSignature=repr(current["pattern geometry"]); target.SewingSignature=repr(current["sewing topology"]); target.AvatarSignature=repr(current["avatar"]); target.ArrangementSignature=repr(current["arrangement"]); target.CollisionParametersSignature=repr(current["collision parameters"]); target.SourcePlacementSignature=repr(current["source placement"]); target.SourceSignature=repr(source_signature(getattr(target,"SourceObject",None),getattr(target,"CollisionDeflection",1.0),getattr(target,"CollisionThickness",0.0)))

def refresh_drape_target(target):
    source=getattr(target,"SourceObject",None)
    if source is None: raise ValueError("drape target source is required")
    target.LifecycleState="REFRESHING"; target.TargetStatus="REFRESHING"; target.InvalidationReason="explicit refresh"; doc=getattr(target,"Document",None)
    if doc is not None: doc.recompute()
    surface=collision_surface(source,float(getattr(target,"CollisionDeflection",1.0)),float(getattr(target,"CollisionThickness",0.0))); target.CollisionVertexCount=len(surface.vertices); target.CollisionTriangleCount=len(surface.triangles); _set_signatures(target,dependency_signatures(target)); target.InvalidationReason=""; target.LifecycleState="READY_FOR_SIMULATION"; target.TargetStatus="READY_FOR_SIMULATION"
    if doc is not None: doc.recompute()
    return target

def invalidate_drape_target(target,reason):
    if target is None: return None
    target.LifecycleState="STALE"; target.TargetStatus="STALE"; target.InvalidationReason=str(reason).strip() or "dependency changed"; return target
    ...
    def execute(self,target):
        try:
            ...
            if not getattr(target,"DependencySignature",""): invalidate_drape_target(target,"collision cache missing")
                ...
                reasons=_changed_reasons(target,current); invalidate_drape_target(target,", ".join(reasons) if reasons else "dependency changed")
            target.TargetStatus=str(getattr(target,"LifecycleState","INVALID"))
        except (AttributeError,TypeError,ValueError,RuntimeError) as exc: invalidate_drape_target(target,"dependency inspection failed: %s"%exc)
...
def create_drape_target(doc,source=None,target_type="FreeCAD Geometry",deflection=1.0,thickness=0.0):
    ...
    for prop,typ,group in (("TargetType","App::PropertyString","Draping"),("SourceObject","App::PropertyLink","Draping"),("PatternDependencies","App::PropertyLinkList","Dependencies"),("SewingDependencies","App::PropertyLinkList","Dependencies"),("AvatarDependencies","App::PropertyLinkList","Dependencies"),("ArrangementDependencies","App::PropertyLinkList","Dependencies"),("CollisionDeflection","App::PropertyFloat","Collision"),("CollisionThickness","App::PropertyFloat","Collision"),("Enabled","App::PropertyBool","Draping"),("CollisionVertexCount","App::PropertyInteger","State"),("CollisionTriangleCount","App::PropertyInteger","State"),("SourceSignature","App::PropertyString","State"),("DependencySignature","App::PropertyString","State"),("PatternSignature","App::PropertyString","State"),("SewingSignature","App::PropertyString","State"),("AvatarSignature","App::PropertyString","State"),("ArrangementSignature","App::PropertyString","State"),("CollisionParametersSignature","App::PropertyString","State"),("SourcePlacementSignature","App::PropertyString","State"),("LifecycleState","App::PropertyEnumeration","State"),("TargetStatus","App::PropertyString","State"),("InvalidationReason","App::PropertyString","State")):
        target.addProperty(typ,prop,group)
    target.LifecycleState=list(STATES); target.LifecycleState="STALE"; target.TargetType=target_type; target.CollisionDeflection=float(deflection); target.CollisionThickness=float(thickness); target.Enabled=True; target.CollisionVertexCount=0; target.CollisionTriangleCount=0; target.TargetStatus="STALE"; target.InvalidationReason="collision cache missing"; target.Proxy=DrapeTargetProxy()
...
def assign_drape_target(target,source,target_type:Optional[str]=None):
    ...
    target.TargetType=kind; target.SourceObject=source; return refresh_drape_target(target)
