...
def target_status(target):
    ...
    if source is None:
        return {"state": "unassigned", "message": "Drape target has no source object", "stale": True, "reason": "source missing"}
    try:
    ...
    authored = str(getattr(target, "SourceSignature", "")); vertices = int(getattr(target, "CollisionVertexCount", 0)); triangles = int(getattr(target, "CollisionTriangleCount", 0))
...
def _install_simulation_guard():
    try:
        ...
        return
    if getattr(SimulationProxy, "_drape_target_guard_installed", False): return
    original = SimulationProxy.execute
    def guarded_execute(self, obj):
        target = getattr(obj, "DrapeTarget", None)
        if target is not None and target_status(target)["stale"]:
...
def refresh_drape_target(target):
    ...
    _install_simulation_guard(); return result
...
def create_drape_target(doc, source=None, target_type="FreeCAD Geometry", deflection=1.0, thickness=0.0):
    ...
    target.addProperty("App::PropertyString", "TargetType", "Draping"); target.addProperty("App::PropertyLink", "SourceObject", "Draping")
    target.addProperty("App::PropertyFloat", "CollisionDeflection", "Collision"); target.addProperty("App::PropertyFloat", "CollisionThickness", "Collision")
    target.addProperty("App::PropertyBool", "Enabled", "Draping"); target.addProperty("App::PropertyInteger", "CollisionVertexCount", "State"); target.addProperty("App::PropertyInteger", "CollisionTriangleCount", "State")
    target.addProperty("App::PropertyString", "SourceSignature", "State"); target.addProperty("App::PropertyString", "TargetStatus", "State"); target.addProperty("App::PropertyString", "InvalidationReason", "State")
    ...
    _install_simulation_guard(); return target
...
def assign_drape_target(target, source, target_type: Optional[str]=None):
    ...
    deflection=float(getattr(target,"CollisionDeflection",1.0)); thickness=float(getattr(target,"CollisionThickness",0.0)); surface=collision_surface(source,deflection,thickness)
    ...
    status=target_status(target); target.TargetStatus=status["state"]; target.InvalidationReason=status["reason"]; _install_simulation_guard(); return target
