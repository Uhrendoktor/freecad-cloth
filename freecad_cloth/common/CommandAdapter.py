...
def register_commands(gui, commands):
    """Register ``name -> callable`` pairs with a FreeCAD GUI module.

    ``gui`` is passed in by the FreeCAD-facing module so importing this adapter
    never imports FreeCAD or Qt and remains safe in headless tests.
    """
