...
def _default_segment_id(piece):
    ...
    sewing_boundary = str(getattr(piece, "SewingBoundary", "")).strip()
    if sewing_boundary:
        return sewing_boundary.split(",", 1)[0].strip()
    return ""
...
def add_mark(doc, mark_type, piece_id, segment_id="", position=0.5, depth=3.0, angle=0.0, length=40.0, text=""):
    ...
    ordinal = 1 + len([
        ...
        if getattr(o, "PatternMarkType", "") == mark_type
        and str(getattr(o, "PieceId", "")) == str(piece_id)
    ])
    name = "%s_%d" % (mark_type, ordinal)
    obj = doc.addObject("App::FeaturePython", name)
    obj.Label = text.strip() or name
    obj.addProperty("App::PropertyString", "PatternMarkId", "Pattern Mark").PatternMarkId = "%s:%s:%d" % (
        str(piece_id), str(mark_type).strip().lower(), ordinal
    )
