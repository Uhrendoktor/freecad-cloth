...
def _piece_construction_marks(piece, pattern):
    """Materialize persisted PatternMark objects into the export DerivedPattern."""
    doc = getattr(piece, "Document", None)
    if doc is None:
        raise ValueError("pattern piece is not attached to a FreeCAD document")
    piece_id = str(getattr(piece, "PieceId", "")).strip()
    ...
    for obj in getattr(doc, "Objects", ()):
        ...
        if not mark_type or str(getattr(obj, "PieceId", "")).strip() != piece_id:
            continue
        mark_id = str(getattr(obj, "PatternMarkId", "") or getattr(obj, "Name", "")).strip()
        if not mark_id:
    ...
    return tuple(sorted(notches, key=lambda value: value.id)), tuple(sorted(marks, key=lambda value: value.id))
