...
def target_is_blocked(target_info):
    """Return whether target lifecycle state must block Step/Run."""
    return str((target_info or {}).get("state", "missing")) in _TARGET_BLOCKED_STATES


def fitting_status(fitting_scene):
    """Summarize persisted Arrange/Fit document facts without inferring validity."""
    ...
    return {
        "state": "present",
        "message": "Arrange/Fit: %s | %d garment piece(s) | %d/%d saved placement(s) | %d arrangement point(s)" % (status, pieces, placements, pieces, points),
...
class SimulationQualityTaskPanel:
    ...
    def _arrange_clicked(self):
        try:
            from freecad_cloth.avatar.FittingCommands import add_selected_pattern_pieces, apply_arrangement_point, create_fitting_scene
    ...
    def _refresh_target(self):
        ...
        try:
            ...
            elif info["state"] in {"invalid", "unassigned"}:
                if target is None:
