...
def fitting_status(fitting_scene):
    if fitting_scene is None:
        return {"state":"missing","message":"No Arrange & Fit scene is present.","pieces":0,"placements":0,"points":0,"can_reset":False}
    ...
    return {"state":"present","message":"Arrange/Fit: %s | %d garment piece(s) | %d/%d saved placement(s) | %d arrangement point(s)"%(status,pieces,placements,pieces,points),
            "pieces":pieces,"placements":placements,"points":points,"can_reset":bool(home)}
...
class SimulationQualityTaskPanel:
    ...
    def __init__(self, scene=None):
        ...
        self.arrange_button.setToolTip("Create or use the existing Arrange & Fit scene, then snap assigned garment pieces to the current DrapeTarget.")
        self.reset_arrangement_button.setToolTip("Restore the saved Arrange & Fit home placements.")
        self.refresh_target_button.setToolTip("Rebuild the current DrapeTarget collision surface through the existing validation path.")
        cform.addRow("Target", self.target_context); cform.addRow("Placement", self.placement_status)
        cform.addRow("", self.arrange_button); cform.addRow("", self.reset_arrangement_button); cform.addRow("", self.refresh_target_button)
    ...
    def _arrange_clicked(self):
        try:
            ...
            if fitting is None:
                ...
                fitting = self._find_fitting_scene()
            if fitting is None:
                raise ValueError("Arrange & Fit scene could not be created")
    ...
    def _refresh_target(self):
        ...
        try:
            ...
            if info["state"] == "disabled":
                ...
                return
            if info["state"] in {"invalid", "unassigned", "missing"}:
                raise ValueError(info["message"])
            if getattr(target, "SourceObject", None) is None:
                raise ValueError(info["message"])
            refresh_drape_target(target); self.scene.Document.recompute()
    ...
    def _refresh(self, message=None):
        ...
        if target_info["state"] == "disabled":
            self.refresh_target_button.setText("Enable target"); self.refresh_target_button.setEnabled(target is not None)
        elif target_info["state"] in {"invalid", "unassigned", "missing"}:
            self.refresh_target_button.setText("Edit target"); self.refresh_target_button.setEnabled(target is not None)
        else:
            self.refresh_target_button.setText("Refresh target"); self.refresh_target_button.setEnabled(bool(target_source) and target_info["state"] != "ready")
        fitting = self._find_fitting_scene()
        ...
        if fit_info["state"] == "missing":
            ...
            self.arrange_button.setText("Snap assigned pieces to target")
        if blocked:
