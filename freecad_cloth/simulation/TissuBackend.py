...
_TISSU_COLLISION_TRIANGLES_DEFAULT = 2048
...
def _tissu_collision_triangle_limit():
    ...
    if value < 16:
        raise ValueError("CLOTH_TISSU_COLLISION_TRIANGLES must be >= 16")
    return value
...
class TissuBackend(ClothSimulationBackend):
    ...
    def __init__(
        ...
        self._collision_surface = (
            coarsen_collision_surface(collision_surface, _tissu_collision_triangle_limit())
            if collision_surface is not None and collision_mode == "mesh"
            else collision_surface
        )
        if collision_surface is not None and self._collision_surface is not collision_surface:
            print(
                "cloth-tissu-collision source_triangles=%d solver_triangles=%d vertices=%d limit=%d"
                % (
                    len(collision_surface.triangles),
                    len(self._collision_surface.triangles),
                    len(self._collision_surface.vertices),
                    _tissu_collision_triangle_limit(),
                ),
                flush=True,
            )
