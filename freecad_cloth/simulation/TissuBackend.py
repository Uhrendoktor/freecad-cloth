...
def _build_signed_collision_bvh(surface, leaf_size=8):
    ...
    # Match the authoritative Tissu mesh registration after the Y/Z handedness reflection.
...
def _signed_collision_guard_correction(position, old_position, closest, normal, thickness, friction=0.85):
    import numpy as np

    if not 0.0 <= float(friction) <= 1.0:
        raise ValueError("signed collision guard friction must be in [0, 1]")
    signed_distance = float(np.dot(position - closest, normal))
    if signed_distance >= float(thickness):
        return None
    corrected = (
        closest + normal * float(thickness)
        if signed_distance < 0.0
        else position
    )
    displacement = corrected - old_position
    normal_velocity = float(np.dot(displacement, normal))
    normal_velocity_vector = normal * normal_velocity
    tangent_velocity = displacement - normal_velocity_vector
    new_displacement = normal_velocity_vector + tangent_velocity * (1.0 - float(friction))
    corrected_old = corrected - new_displacement
    return corrected, corrected_old, max(0.0, -signed_distance)
...
class TissuBackend(ClothSimulationBackend):
    ...
    def __init__(
        ...
        self._signed_collision_bvh = (
            ...
            if self._signed_collision_guard and self._collision_mode == "mesh" and self._collision_surface is not None
            else None
        )
