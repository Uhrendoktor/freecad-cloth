...
def create_free_sewing_from_selection():
    ...
    piece_a, edge_a = _selected_pattern_edges()[0]
    piece_b, edge_b = _selected_pattern_edges()[1]
    side_a = (SewingMember(str(piece_a.PieceId), edge_a),)
    side_b = (SewingMember(str(piece_b.PieceId), edge_b),)
    lengths = {
        (str(piece_a.PieceId), edge_a): _edge_length(piece_a, edge_a),
        (str(piece_b.PieceId), edge_b): _edge_length(piece_b, edge_b),
    }
    existing = {str(getattr(obj, "RelationshipId", "")) for obj in doc.Objects}
    index = 1
...
def start_staged_free_sewing_creation():
    """Open the public staged Preview/Commit/Cancel flow for Free Sewing."""
    from freecad_cloth.sewing.SewingCreationGui import show_sewing_creation_task
    return show_sewing_creation_task("free")
...
try:
    ...
    class _FreeSewingCommand:
        def Activated(self): return start_staged_free_sewing_creation()
        def IsActive(self):
            try:
                import FreeCAD as App
