...
class SewingTaskPanel:
    ...
    def _update_seam_color(self):
        ...
        seam_ids=[str(getattr(candidate,"SeamId","")).strip() for candidate in getattr(document,"Objects",()) if str(getattr(candidate,"SeamId","")).strip()]
    ...
    def _refresh(self):
        self._update_seam_color(); self.status.setText(str(self.obj.Status)); self.lengths.setText("%.2f / %.2f mm (Δ %.2f)"%(float(self.obj.LengthA),float(self.obj.LengthB),float(self.obj.LengthDifference)))
