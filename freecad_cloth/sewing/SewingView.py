...
def _seam_color_for_id(seam_id):
    """Return one stable presentation color derived only from canonical seam identity."""
...
def seam_color_map(seam_ids):
    ...
    Color assignment deliberately does not depend on the set, document order,
    generated mesh topology, or seam creation order. Adding/removing another
    seam therefore cannot recolor an existing seam identity.
    """
