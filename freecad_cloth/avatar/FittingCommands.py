...
def _fitting_target(scene):
    """Return the persistent DrapeTarget owned by the fitting workflow, fail-closed on ambiguity."""
...
def _piece_world_samples(piece, deflection=1.0):
    ...
    if shape is not None and not shape.isNull():
        ...
        for vertex in getattr(shape, "Vertexes", ()) or ():
            point = placement.multVec(vertex.Point) if placement is not None else vertex.Point
...
def snap_pattern_pieces_to_target(pieces=None, clearance=None, max_translation=750.0, sample_deflection=1.0):
    """Place garment pieces by bounded rigid translation outside the authoritative DrapeTarget."""
    import FreeCAD as App
    ...
    required_clearance = (
        max(2.0, float(getattr(target, "CollisionThickness", 0.0)))
        if clearance is None else max(0.0, float(clearance))
    )
        ...
        for piece in selected:
            ...
            for _attempt in range(3):
                ...
                if total_travel + deficit > guard + 1e-9:
                    raise ValueError(
                        "target-aware placement for %s cannot satisfy %.3f mm clearance within the translation guard"
                        % (piece.Label, required_clearance)
                    )
                ...
                piece.Placement = App.Placement(
                    App.Vector(float(current_base.x) + correction_delta[0], float(current_base.y) + correction_delta[1], float(current_base.z) + correction_delta[2]),
                    current.Rotation,
                )
            ...
            if abs(float(getattr(piece.Placement.Rotation, "Angle", 0.0)) - float(getattr(placement.Rotation, "Angle", 0.0))) > 1e-9:
                raise ValueError("target-aware placement unexpectedly changed garment rotation")
            results.append({
