...
def add_selected_pattern_pieces():
    ...
    for piece in pieces:
        ...
        value = PiecePlacement(str(piece.PieceId), (float(base.x), float(base.y), float(base.z)), float(placement.Rotation.Angle), (float(axis.x), float(axis.y), float(axis.z)))
...
def set_garment_anchors(anchors):
    ...
    for anchor in anchors or ():
        ...
        item.validate()
        values.append(item)
    scene.GarmentAnchors = [item.to_string() for item in sorted(values, key=lambda a: (a.piece_id, a.name))]
    doc.recompute()
    return scene


def target_aware_place_piece(piece, target, anchors, clearance=8.0, max_translation=600.0, max_rotation=45.0):
    """Place one piece rigidly from garment anchors onto the persistent DrapeTarget."""
    import FreeCAD as App
    ...
    selected = tuple(sorted((item for item in selected if str(item.piece_id) == str(piece.PieceId)), key=lambda item: item.name))
    if not selected:
