...
class AvatarTaskPanel:
    ...
    def __init__(self, avatar=None):
        ...
        placement_help = QtWidgets.QLabel(
            "For initial garment placement, select a PatternPiece in the 3D view, "
            "then Snap to Drape Target. Reset Arrangement restores the saved home placement."
        )
        ...
        self.snap_button.setToolTip(
            "Move the selected PatternPiece to the current DrapeTarget with bounded surface clearance."
        )
    ...
    def _snap_selected_to_target(self):
        ...
        if active is None:
            self._refresh_status("Open a document before snapping a garment piece.")
            return
        piece = next(
            (obj for obj in self.Gui.Selection.getSelection() if getattr(obj, "PatternType", "") == "PatternPiece"),
            None,
        )
        if piece is None:
            self._refresh_status("Select a PatternPiece in the 3D view, then Snap to Drape Target.")
            return
        target = active.Document.getObject("DrapeTarget")
        if target is None:
            self._refresh_status("Create or assign a DrapeTarget before snapping a garment piece.")
            return
        try:
            from freecad_cloth.avatar.FittingCommands import snap_piece_to_drape_target
            result = snap_piece_to_drape_target(piece, target)
        except (RuntimeError, TypeError, ValueError) as exc:
            self._refresh_status("Garment placement blocked: %s" % exc)
            return
        self._refresh_status(
            "Garment snapped to target: clearance %.2f mm; translation %.2f mm."
            % (float(result["distance_after"]), float(result["translation"]))
        )
        self._fit_view()
