...
def solve_rigid_z(source_points, target_points, max_translation=600.0, max_rotation=45.0):
    ...
    max_motion = max(_norm(_sub(a, b)) for a, b in zip(transformed, source))
    if max_motion > float(max_translation):
        raise TargetPlacementError("target-aware rigid motion exceeds the configured bound")
