"""Deterministic solver-neutral rigid garment placement against a collision surface."""
...
class TargetPlacementError(ValueError):
    """Raised when a target-aware starting placement cannot be proven safe."""
...
def _triangle_data(surface, index):
    ...
    # Orient the triangle normal away from the deterministic surface center.
...
def target_surface_anchor(surface, point, expected_normal, ambiguity_tolerance=1e-6):
    """Return the deterministic nearest surface location with an aligned normal."""
    hits = sorted(
        _candidate_hits(surface, point, expected_normal),
        key=lambda hit: (round(hit.distance, 12), hit.triangle_index),
    )
    if not hits:
        raise TargetPlacementError("no target surface location matches the garment wrap direction")
    best = hits[0]
    if (
        len(hits) > 1
        and abs(hits[1].distance - best.distance) <= float(ambiguity_tolerance)
        and _dot(best.normal, hits[1].normal) < 0.20
    ):
...
def solve_rigid_z(source_points, target_points, max_translation=600.0, max_rotation=45.0):
    """Solve a deterministic rigid translation plus world-Z rotation for anchor pairs."""
    ...
    residual_max = max(_norm(_sub(a, b)) for a, b in zip(transformed, target))
    return RigidDelta(translation, degrees(angle), residual_max)
...
def minimum_surface_clearance(surface, points):
    """Return minimum signed outward clearance from points to a closed target surface."""
    minimum = None
    for point in points:
        hits = sorted(
...
def assert_minimum_surface_clearance(surface, points, required_clearance):
    ...
    if actual < required - 1e-6:
        raise TargetPlacementError(
            "target-aware step-0 clearance %.6f mm is below required %.6f mm" % (actual, required)
        )
    return actual
