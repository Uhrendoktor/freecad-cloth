...
`Create fitting scene → assign avatar/target → add PatternPieces → create arrangement points → apply arrangement or **Snap to Drape Target** → inspect/reset → create simulation`.

Arrangement points are avatar-relative fitting metadata. They establish the garment start pose; **Snap to Drape Target** then applies bounded rigid translation to the authoritative target surface, with rollback on failure. They are preparation state, not solver state.

A reset is always available: use **Reset Arrangement** to restore the saved pre-arrangement PatternPiece placements.

For mannequin fitting, the current canonical tunic fixture seeds the panels from avatar landmarks and then **Snaps to Drape Target** before the first solver step; the accepted step-0 surface clearance is measured and the garment path does not rely on global pinning as a substitute for fitting.
