...
3. In **Cloth Sewing → Fitting & Avatar**, create a fitting scene and add the PatternPieces. Assign the current DrapeTarget, then run **Target-aware Arrange** on a selected PatternPiece. When no anchors have been saved, the command derives deterministic shoulder anchors from the selected piece; saved anchors and wrap directions can override that default. The action fails closed for a missing/stale/ambiguous target or bounded-transform violation and rolls back the PatternPiece, linked Sketch and persisted fitting state on failure.
4. In **Cloth Simulation**, verify the same DrapeTarget is attached to the simulation. The target is authoritative for collision; stale or invalid target state blocks use rather than silently falling back to an avatar proxy.
5. Set simulation quality, fabric presentation and pinning mode. **Automatic** preserves legacy behavior, **Explicit** uses only `PinSelection`, and **None** creates a genuinely pinless solver scene. Stale or non-finite states are fail-closed.
