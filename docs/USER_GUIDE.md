...
3. In **Cloth Simulation**, select or rebuild a DrapeTarget. For the native mannequin, use **Arrange garment on mannequin** for the two-panel tunic before simulation; this is target-relative placement and does not require global pins. A target can also be supported generic FreeCAD geometry.
