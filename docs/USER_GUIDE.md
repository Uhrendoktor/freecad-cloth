...
3. In **Cloth Sewing → Fitting & Avatar**, create a fitting scene, add the PatternPieces, assign the persistent DrapeTarget, and run **Snap Pieces to Target** on the authored garment selection. Multiple selected pieces are moved together so their authored spacing is preserved; the action is bounded and rolls back on any failed target/clearance validation.
4. In **Cloth Simulation**, verify the same DrapeTarget is attached to the simulation, then set simulation quality and fabric presentation. Physical material parameters affect the solver; color, roughness, specular response and transparency affect viewport rendering.
