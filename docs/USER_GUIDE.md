...
The current simulation quality panel also exposes **Pinning mode** with **Automatic**, **Explicit**, and **None**. Automatic preserves legacy behavior, Explicit uses only PinSelection, and None creates zero solver pins. The panel still does not provide a mesh-vertex picking tool for authoring PinSelection, so the canonical blanket fixture remains the executable proof for explicit corner pinning.
...
The current acceptance fixture uses **Pinning mode = None**, with zero solver pins, and places the front/back panels target-relatively before simulation using the persistent DrapeTarget as the geometric authority. This is a fixture contract, not evidence that the general Fitting Scene UI performs automatic CLO-style snapping.
