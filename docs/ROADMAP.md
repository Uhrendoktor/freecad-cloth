...
- The three README turntable GIFs are 73 frames at 640x480 with uniform 8cs frame timing; the canonical workflow asserts that timing fail-closed. Representative rendered frames were inspected, and the publication audit confirms those live structural/timing properties, but it does not establish byte identity between the published GIF Git blobs and the downloaded GIF artifacts from canonical runs #3976/#3979.
...
- The publisher job #108293224713 populated `docs/screenshots` at commit `2a7fd1e`; the published tunic-front PNG matches the final artifact blob. Exact-byte GIF provenance remains unverified because the downloaded #3976/#3979 GIF bytes do not reproduce the published GIF Git blob SHAs.
