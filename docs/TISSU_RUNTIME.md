...
Canonical GUI simulation uses a patched Tissu 1.1.0 runtime because the pinned upstream `MeshCollider::resolve()` implementation derives its collision normal directly from the particle-to-closest-point vector. For a particle that has crossed into a closed mesh, that vector points inward. The repository-controlled patch corrects that direction only for closed, consistently wound collision surfaces; open/non-manifold meshes retain the legacy behavior.
...
The script fail-closes on an upstream commit mismatch, a patch-application mismatch, or a failing native `MeshCollider` test. It does not modify any simulation timeout, seam threshold, collision triangle cap, solver quality, or visual acceptance gate.
