...
The canonical acceptance fixture explicitly maps the two opposite top-edge corners to the simulation's pin selection. The current quality task panel does not expose a click-to-pin mesh editor, so this example is not documentation for a missing UI feature. Treat the executable fixture as the definitive automated installation proof.
...
The validated tunic fixture uses authored shoulder pinning on the front panel while the back panel is not globally pinned. This is a fixture-specific acceptance detail; it is not a prescription for all garments and does not imply automatic pinless snapping to the mannequin.
