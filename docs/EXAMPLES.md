...
The tunic is the full garment acceptance scenario. It uses two pattern pieces, semantic sewing, target-aware rigid placement against the production MakeHuman mannequin, persisted quality/material settings, diagnostics, save/reload and invalidation. The visual fixture records the snap clearance before simulation so an avatar-intersection failure cannot be hidden by later solver steps.
