...
| Candidate | Capability | Proposed role | Dependency posture | Decision |
|---|---|---|---|---|
| Tissu | XPBD cloth, bending, pins, stitches, mesh/kinematic collision, self-collision | Optional simulation backend | Isolated adapter; never mandatory initially | Benchmark |
| PositionBasedDynamics | XPBD/PBD, mesh/SDF collision, Python bindings | Physics comparator / optional backend | Sandbox only | Benchmark later |
| libigl | geometry processing, parametrization, distances, remeshing | Analysis/mesh utilities where OCCT is insufficient | Optional | Evaluate |
...
| Shapely/GEOS | robust 2D polygon operations | Diagnostics/export preprocessing | Optional | Evaluate selectively |
| trimesh | mesh validation and proximity | Acceptance diagnostics and backend comparison | Optional/lazy | Implement first |
| meshio | broad mesh I/O | Benchmark fixtures/interoperability | Optional/development-only | Low priority |
...
| Seamly2D / FreeSewing | garment-specific pattern workflows | Research/interoperability reference | Avoid embedding external core | Study only |
...
1. `trimesh` validation/proximity adapter (#481) provides machine-checkable mesh health and garment/target clearance metrics.
2. Tissu evaluation (#479) uses the same metrics plus constraint/collision/determinism benchmarks.
