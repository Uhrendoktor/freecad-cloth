...
class _ShuffledSketch:
    Geometry = (
        _LineSegment((0, 0), (10, 0)),
        _LineSegment((0, 10), (0, 0)),
        _LineSegment((10, 0), (10, 10)),
        _LineSegment((10, 10), (0, 10)),
    )
    SemanticEdgeIds = (
        "piece:edge:0",
        "piece:edge:1",
...
def test_sketcher_semantic_seam_reference_preserves_native_provenance_signature():
    ...
    record = next(record for record in _edge_records(piece) if record["id"] == edge_id)
