...
def test_seam_color_surface_contract_carries_identity_to_sewing_operations():
    ...
    pattern_source = (Path(__file__).resolve().parents[1] / "freecad_cloth" / "pattern" / "PatternObjects.py").read_text(encoding="utf-8")
    assert "refresh_seam_colors(obj.Document)" in pattern_source
