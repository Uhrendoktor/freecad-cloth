...
def run_acceptance():
    ...
    try:
        ...
        if not hasattr(quality_panel, "arrange_fit_button") or not hasattr(quality_panel, "snap_to_target_button") or not hasattr(quality_panel, "refresh_target_button"):
            raise RuntimeError("Simulation quality task panel did not expose the Arrange / Fit / target bridge")
        snap_command_available = "ClothFitting_SnapPiecesToTarget" in Gui.listCommands()
        if quality_panel.snap_to_target_button.isEnabled() != snap_command_available:
            raise RuntimeError("Simulation panel did not fail closed on public target-aware placement command availability")
        ...
        position_piece(
            piece,
            original_base.x + 10.0,
            original_base.y,
            original_base.z,
            rotation_z=float(original_rotation.Angle),
        )
        doc.recompute()
        ...
        print("arrange-fit-ui=passed states=present,reset,recovered", flush=True)
