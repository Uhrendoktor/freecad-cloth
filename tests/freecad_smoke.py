...
def main():
    ...
    avatar_doc=App.newDocument("AvatarSmoke"); mannequin=create_avatar(); assert mannequin.AvatarStatus=="Valid" and mannequin.Shape.isValid() and mannequin.Shape.Volume>0; assert mannequin.AvatarMeshProvider=="makehuman-hm08" and mannequin.AvatarMeshLicense=="CC0" and int(mannequin.MeshVertexCount)>100 and int(mannequin.MeshTriangleCount)>100; original=mannequin.Shape.Volume; set_avatar_measurements(chest=1100); assert mannequin.Shape.Volume!=original; set_avatar_pose("sewing"); set_avatar_skin_offset(6.0); assert mannequin.PosePreset=="sewing" and abs(float(mannequin.SkinOffset)-6.0)<1e-9 and mannequin.CollisionProxy is not None; avatar_doc.recompute()
    checkpoint("FREECAD_SMOKE MANNEQUIN")
    with tempfile.TemporaryDirectory() as directory:
        path=str(Path(directory)/"avatar.FCStd"); avatar_doc.saveAs(path); App.closeDocument(avatar_doc.Name); reopened=App.openDocument(path); restored=reopened.getObject("ClothAvatar"); assert restored is not None and restored.AvatarStatus=="Valid"; assert restored.AvatarMeshProvider=="makehuman-hm08"; assert restored.AvatarMeshLicense=="CC0"; assert restored.PosePreset=="sewing"; assert abs(float(restored.Chest)-1100.0)<1e-9; assert abs(float(restored.SkinOffset)-6.0)<1e-9; App.closeDocument(reopened.Name)
    checkpoint("FREECAD_SMOKE PERSISTENCE")
    print("FreeCAD workbench, Sketcher, sewing correspondence, drape target, MakeHuman humanoid drape, and production avatar smoke test passed", flush=True)
