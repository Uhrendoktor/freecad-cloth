...
def main():
    ...
    # Native Sketcher is an explicit, persistent geometry authority rather than a copy-only adapter.
    ...
    scene.PinSelection=["0","not-an-index","79"];scene.SeamSelection=["7-8","bad-pair","39-40"];reset_scene(scene);assert scene.Steps==0 and scene.SimulatedTime==0.0 and scene.FiniteState;assert scene.ClothPieces==[doc.getObject("DrapePanelA"),doc.getObject("DrapePanelB")];assert scene.AvatarProxy==doc.getObject("AvatarCollision");print("FreeCAD workbench, native Sketcher authority, OCCT adapter, placement and humanoid drape smoke test passed")
