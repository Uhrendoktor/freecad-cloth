...
def main():
    ...
    Gui.Selection.clearSelection();Gui.Selection.addSelection(obj);sketch=create_pattern_sketch();assert obj.Sketch==sketch;assert obj.GeometryMode=="Sketch";assert obj.GeometryAuthority=="Sketcher";assert sketch.GeometryAuthority=="Sketcher";assert sketch.GeometryContractVersion=="1";assert list(sketch.SemanticEdgeIds)==["pattern-piece-1:edge:%d"%i for i in range(4)];assert obj.Visibility is False and sketch.Visibility is True
    old_length=obj.Shape.BoundBox.XLength
    import Sketcher
    constraint_index=sketch.addConstraint(Sketcher.Constraint("Distance",0,120.0));sketch.renameConstraint(constraint_index,"PatternWidth");sketch.setDatum(constraint_index,App.Units.Quantity("140 mm"));doc.recompute()
    ...
    assert any(getattr(c,"Name","")=="PatternWidth" for c in sketch.Constraints)
    with tempfile.TemporaryDirectory() as directory:
        path=str(Path(directory)/"sketch-authority.FCStd")
        doc.recompute();doc.saveAs(path);App.closeDocument(doc.Name);doc=App.openDocument(path);obj=doc.getObject("PatternPiece");sketch=doc.getObject("PatternSketch_pattern_piece_1");assert obj is not None and sketch is not None;assert obj.Sketch==sketch;assert obj.GeometryAuthority=="Sketcher";assert sketch.GeometryAuthority=="Sketcher";assert sketch.GeometryContractVersion=="1";assert list(sketch.SemanticEdgeIds)==["pattern-piece-1:edge:%d"%i for i in range(4)];assert abs(float(obj.Width)-140.0)<1e-9;assert any(getattr(c,"Name","")=="PatternWidth" for c in sketch.Constraints)
    ...
    print("FreeCAD workbench, Sketcher authority/persistence, sewing correspondence, drape target, humanoid drape, and parametric avatar smoke test passed")
