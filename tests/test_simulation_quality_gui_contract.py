...
def test_arrange_fit_bridge_reports_persisted_state_without_validity_claims():
    ...
    source = SimpleNamespace(Name="", Placement=None, Shape=None)
    target = SimpleNamespace(
        Enabled=True,
        TargetType="FreeCAD Geometry",
        SourceObject=source,
        CollisionVertexCount=1,
    ...
    simulation = SimpleNamespace(
        DrapeTarget=target,
        Document=SimpleNamespace(Objects=(fitting,)),
    )
    message, can_reset = fitting_stage_status(simulation)

    assert message.startswith("Arrange / Fit: Ready | 2 pieces assigned | 2/2 saved placement(s) | 1 arrangement point(s) | target: ready")
    assert can_reset is True
    assert "valid" not in message.lower()
    assert "invalid" not in message.lower()


def test_arrange_fit_bridge_keeps_reset_syntax_and_uses_existing_public_commands():
    assert r"def reset_arrangement(self):\n        try:" not in GUI
    ...
    assert 'self.Gui.runCommand("ClothFitting_SnapPiecesToTarget", 0)' in GUI
    assert "snap_pattern_pieces_to_target" not in GUI
