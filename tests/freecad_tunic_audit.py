...
seam_check = """    backend_state = scene.Proxy._base_or_restore()
    ...
    for seam, piece_a, piece_b in seam_records:
        ...
        if not pairs: raise RuntimeError("authoritative seam check cannot resolve exact solver pairs for %s" % seam.SeamId)
        for ga, gb in pairs:
