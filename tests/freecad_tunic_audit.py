...
replacements = {
    ...
    '        seam_obj = next(o for o in doc.Objects if getattr(o, "SeamId", "") == seam_id)\n        seam_records.append((seam_obj, front, back))':
        '        seam_obj = next(o for o in doc.Objects if getattr(o, "SeamId", "") == seam_id)\n        if seam_id == "TunicRightShoulder" and (str(getattr(seam_obj, "EdgeAId", "")), str(getattr(seam_obj, "EdgeBId", ""))) != (f"{front.PieceId}:edge:2", f"{back.PieceId}:edge:2"):\n            raise RuntimeError("right shoulder seam did not resolve to authored edge 2")\n        if seam_id == "TunicLeftShoulder" and (str(getattr(seam_obj, "EdgeAId", "")), str(getattr(seam_obj, "EdgeBId", ""))) != (f"{front.PieceId}:edge:6", f"{back.PieceId}:edge:6"):\n            raise RuntimeError("left shoulder seam did not resolve to authored edge 6")\n        seam_records.append((seam_obj, front, back))',
