...
_replacement_specs = (
    (
        r"(?m)^(\\s*)clearance = max\\(20\\.0, 0\\.08 \\* body_depth\\)(?:;)?$",
        r"\\1clearance = max(8.0, 0.025 * body_depth);",
        "clearance",
    ),
    (
        r"(?m)^(\\s*)y = \\(shoulder_left\\.y \\+ shoulder_right\\.y\\) / 2\\.0 - clearance$",
        r"\\1y = min(target_ys) - clearance",
        "front placement",
    ),
    (
        r"(?m)^(\\s*)y = \\(shoulder_left\\.y \\+ shoulder_right\\.y\\) / 2\\.0 \\+ clearance$",
