...
def main():
    ...
    assert all(0 <= stitch.vertex_a < len(first.vertices) for stitch in constraints.stitches)
    assert all(0 <= stitch.vertex_b < len(second.vertices) for stitch in constraints.stitches)

    doc = App.newDocument("MeshPartSmoke")
    doc.addObject("PartDesign::Feature", "MeshPartAdapterProbe")
    doc.recompute()
    doc.close()
