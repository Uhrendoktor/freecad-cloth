...
def test_canonical_concurrency_cancels_stale_pr_runs():
    workflow = (ROOT / ".github" / "workflows" / "canonical-execution.yml").read_text(encoding="utf-8")
    assert "format('canonical-{0}-pr-{1}', github.workflow, inputs.pull_request_number)" in workflow
    assert "format('canonical-broker-{0}-pr-{1}', github.workflow, github.event.pull_request.number)" in workflow
