...
def test_legacy_integer_seam_edge_keeps_legacy_boundary_ordinal():
    ...
    records = [{"id": f"legacy:edge:{index}", "points": ((float(index), 0.0), (float(index + 1), 0.0)), "provenance": ("PatternIR", "Legacy", "line", (0.0, 1.0), ())} for index in range(4)]
    original = pattern_objects._edge_records; pattern_objects._edge_records = lambda _piece: records
    try:
