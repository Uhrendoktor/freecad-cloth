...
def _panel_visible(panel):
    """Use FreeCAD's TaskDialog state; panel.form may be reparented/wrapped."""
    form = getattr(panel, "form", None)
    try:
        active = Gui.Control.activeDialog()
        if active is not None:
            active_form = getattr(active, "form", None)
...
def show_task(panel, state_name):
    ...
    try:
        active = Gui.Control.activeDialog() is not None
    except Exception:
        active = False
    progress("task-panel=%s visible=%s active_dialog=%s docks=%s" % (state_name, visible, active, ",".join(docks())))
    if not visible:
...
def add_sewing_arrows(doc, front, back):
    ...
    for name, x0, x1 in (("Sew direction A", front.Placement.Base.x + 140.0, front.Placement.Base.x + 128.0),
        ...
        head = FC.Vector(x0, 45.0, 0.9)
        tail = FC.Vector(x1, 45.0, 0.9)
        left = FC.Vector(x0 - (3 if x0 < x1 else -3), 49.0, 0.9)
        right = FC.Vector(x0 - (3 if x0 < x1 else -3), 41.0, 0.9)
