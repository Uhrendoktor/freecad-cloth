...
def simulation():
    ...
    for batch in (15,15,15):
        simulation_panel.step(batch); doc.recompute(); events()
    if int(scene.Steps) != 45 or float(scene.SimulatedTime) <= 0.0 or not bool(scene.FiniteState): raise RuntimeError("simulation did not reach a finite 45-step state")
    ...
    for direction, method_name in (("front","viewFront"),("rear","viewRear"),("left","viewLeft"),("right","viewRight"),("top","viewTop"),("bottom","viewBottom")):
        getattr(view, method_name)(); view.fitAll(); events(); save("cloth-simulation-draped-%s.png" % direction, "Simulation Workbench draped %s" % direction, "same sewn tunic after 45 real steps; six-side audit from native Sketcher pattern sources")
