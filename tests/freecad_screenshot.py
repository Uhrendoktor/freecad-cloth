...
def activate_workbench(internal_name, toolbar_name, expected_commands):
    ...
    Gui.activateWorkbench(internal_name); Gui.updateGui()
    window = Gui.getMainWindow()
    if window is None:
    ...
    for bar in window.findChildren(QtWidgets.QToolBar):
        if bar.windowTitle() == toolbar_name:
            bar.show()
    Gui.updateGui()
    active = Gui.activeWorkbench().name(); visible = toolbars()
    ...
    if active != internal_name: raise RuntimeError("failed to activate %s (active=%s)" % (internal_name, active))
    if toolbar_name not in visible: raise RuntimeError("toolbar is not visible: %s" % toolbar_name)
    if missing: raise RuntimeError("commands are not registered: %s" % ",".join(missing))
...
def show_task(panel, state_name):
    ...
    panel.form.setParent(window); panel.form.setGeometry(8, 55, 360, 560); panel.form.show(); panel.form.raise_(); panel.form.activateWindow()
    ...
    if not visible: raise RuntimeError("task panel did not become visible: %s" % state_name)


def panel_text(panel):
    if panel is None or getattr(panel, "form", None) is None: return ""
    texts = []
...
def require_panel(panel, state_name, required):
    if not panel.form.isVisible(): raise RuntimeError("task panel is not visible: %s" % state_name)
    text = panel_text(panel); missing = [item for item in required if item not in text]
    progress("panel-content=%s missing=%s" % (state_name, ",".join(missing)))
    if missing: raise RuntimeError("task panel %s is missing visible text: %s" % (state_name, ",".join(missing)))
...
def add_line_feature(doc, name, segments, width=4.0):
    ...
    feature.Shape = Part.makeCompound([Part.makeLine(a, b) for a, b in segments]); feature.ViewObject.LineWidth = width; feature.ViewObject.LineColor = (0.85, 0.15, 0.15)
    return feature
...
def image_is_useful(path, state_name):
    ...
    pixmap = Gui.getMainWindow().grab(); image = pixmap.toImage()
...
def save_window(filename, state_name, proof):
    ...
    if image.isNull() or not image.save(path): raise RuntimeError("failed to save full-window screenshot: %s" % path)
...
try:
    # FreeCAD already imports InitGui.py during GUI startup. Re-executing it caused
    # duplicate workbench registration (`already exists`) and icon warnings.
    import InitGui
    ...
    progress("import-error"); progress(traceback.format_exc()); raise


def run_scenario():
    doc = None; sim_doc = None; pattern_panel = None; sewing_panel = None; sim_panel = None
    try:
        progress("scenario-start"); open(DEBUG, "w", encoding="utf-8").close(); open(MANIFEST, "w", encoding="utf-8").close()
        window = Gui.getMainWindow()
        if window is not None: window.show(); window.raise_(); window.activateWindow(); window.resize(1280, 720); Gui.updateGui(); progress("main-window=%sx%s" % (window.width(), window.height()))
        ...
        Gui.Selection.clearSelection(); Gui.Selection.addSelection(front); set_top_camera(); save_window("cloth-pattern-design.png", "Pattern Workbench", "two 140x90 mm pieces; native 10 mm seam allowance; task-panel dimensions/allowance; notch and grainline markers"); pattern_panel.form.hide()
        ...
        Gui.Selection.clearSelection(); Gui.Selection.addSelection(sewing); sewing_panel = SewingTaskPanel(sewing); show_task(sewing_panel, "Sewing Workbench")
        ...
        set_top_camera(); save_window("cloth-sewing.png", "Sewing Workbench", "real seam/sewing geometry; uniform correspondence diagnostics; stitch count and length comparison; deterministic direction arrows"); sewing_panel.form.hide()
        ...
        sim_front.Placement.Base.x = -150.0; sim_back.Placement.Base.x = 10.0; sim_seam = add_seam(sim_doc, Seam(str(sim_front.PieceId), 1, str(sim_back.PieceId), 3, id="SimFrontBack", alignment="uniform", stitch_group="MainSeam"))
        ...
        activate_workbench("ClothSimulationWorkbench", "Cloth Simulation", ["ClothSimulation_Edit"]); sim_panel = SimulationQualityTaskPanel(scene); show_task(sim_panel, "Simulation Workbench arranged")
        require_panel(sim_panel, "Simulation Workbench arranged", ["Simulation quality", "Preset", "Fabric", "Collision", "Simulation steps", "Step", "Run 30", "Reset", "State:"]); set_axon_camera(); save_window("cloth-simulation-arranged.png", "Simulation Workbench arranged", "humanoid avatar plus two deterministic arranged garment panels at StartHeight; simulation controls and ready state")
        for batch in (6, 6, 6, 6): sim_panel.step(batch); sim_doc.recompute(); progress("simulation-step batch=%d total=%d time=%.4f particles=%d finite=%s" % (batch, scene.Steps, scene.SimulatedTime, scene.ParticleCount, scene.FiniteState))
        ...
        require_panel(sim_panel, "Simulation Workbench draped", ["State:", "24", "particles", "Fast"]); set_axon_camera(); save_window("cloth-simulation-draped.png", "Simulation Workbench draped", "same avatar/garment scene after 24 real simulation steps; non-empty draped meshes; finite state and elapsed simulation time"); progress("scenario-complete")
    except Exception:
