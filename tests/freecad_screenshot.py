...
def _style_preview(preview):
    preview.ViewObject.DisplayMode = "Flat Lines"; preview.ViewObject.ShapeColor = (0.92, 0.18, 0.10); preview.ViewObject.LineColor = (0.35, 0.02, 0.01); preview.ViewObject.LineWidth = 2.0

def _make_preview_from_mesh(doc, drape, fallback):
    import Part
    try:
        shape = Part.Shape(); shape.makeShapeFromMesh(drape.Mesh.Topology, 0.05)
        if not shape.isNull():
            fallback.Shape = shape; fallback.Placement = App.Placement(); log("garment-preview-source=drape-mesh"); return
    except Exception as error: log("garment-preview-mesh-fallback=%r" % (error,))
...
def _tunic_outline(width, height):
    return [(0.10 * width, height), (0.90 * width, height), (1.00 * width, 0.72 * height), (1.00 * width, 0.00), (0.00, 0.00), (0.00, 0.72 * height)]

def _face_from_outline(outline):
    import Part
    points = [App.Vector(x, y, 0.0) for x, y in outline]; points.append(points[0]); return Part.Face(Part.makePolygon(points))
...
def simulation():
    ...
    if scene.DrapeTarget is None: raise RuntimeError("visual fixture did not create a production DrapeTarget")
    ab = avatar.Mesh.BoundBox
    cx = (ab.XMin + ab.XMax) / 2.0; cy = (ab.YMin + ab.YMax) / 2.0; x_span = ab.XMax - ab.XMin; y_span = ab.YMax - ab.YMin; z_span = ab.ZMax - ab.ZMin; plane_span = min(x_span, y_span); flat_avatar = z_span < 0.35 * max(x_span, y_span)
    garment_width = max(900.0, 0.55 * plane_span); garment_height = max(1200.0, 0.80 * plane_span); outline = _tunic_outline(garment_width, garment_height)
    garment = create_pattern_piece_from_parameters("VisualTunic", garment_width, garment_height, 10.0, 0.0); garment.Label = "Simple Tunic Panel"; garment.DraftingBoundary = repr(outline); garment.SewingOutline = repr(outline)
    ...
    preview = doc.addObject("Part::Feature", "GarmentVisualPreview"); preview.Label = "Garment Preview: Simple Tunic"; preview.Shape = _face_from_outline(outline); preview.Placement = App.Placement(App.Vector(garment.Placement.Base.x, garment.Placement.Base.y, scene.StartHeight), App.Rotation()); _style_preview(preview)
    _style_garment(drape, "Drape: Simple Tunic Panel"); avatar.ViewObject.Visibility = True; doc.recompute()
    log("garment-size=%.1fx%.1f avatar-span=%.1fx%.1f flat=%s start-height=%.1f top-pins=%s" % (garment_width, garment_height, plane_span, z_span, flat_avatar, scene.StartHeight, top_pins))
