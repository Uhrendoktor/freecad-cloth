...
def _panel_visible(panel):
    ...
    if form is None:
        return False
    try:
        if form.isVisible() or form.isVisibleTo(Gui.getMainWindow()):
            return True
    except Exception:
        pass
    window = Gui.getMainWindow()
    ...
    for widget in window.findChildren(QtWidgets.QWidget):
        try:
            if widget.isVisible() and (widget is form or form in widget.findChildren(QtWidgets.QWidget)):
                return True
        except Exception:
            continue
    return False
...
def run_scenario():
    ...
    try:
        ...
        set_top_camera(); save_window("cloth-sewing.png", "Sewing Workbench", "real seam/sewing geometry; uniform correspondence diagnostics; stitch count and length comparison; deterministic direction arrows"); Gui.Control.closeDialog()
...
window = Gui.getMainWindow()
if window is not None:
    window.close(); progress("main-window-closed")
app = QtWidgets.QApplication.instance()
if app is not None:
    app.quit(); app.processEvents()
