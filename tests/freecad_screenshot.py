...
def canonical_garment_e2e():
    load_and_run(os.path.join(ROOT, "tests", "freecad_garment_e2e_smoke.py"), "freecad_garment_e2e_smoke")
    log("canonical-garment-e2e=passed")


def simulation_quality_acceptance():
    load_and_run(os.path.join(ROOT, "tests", "freecad_simulation_quality_acceptance.py"), "freecad_simulation_quality_acceptance")
    log("simulation-quality-acceptance=passed")


def avatar_provider_acceptance():
    load_and_run(os.path.join(ROOT, "tests", "freecad_avatar_acceptance.py"), "freecad_avatar_acceptance")
...
def _tunic_outline(width, height):
    return [
        (0.10 * width, height), (0.90 * width, height),
        (1.00 * width, 0.72 * height), (1.00 * width, 0.00),
        (0.00, 0.00), (0.00, 0.72 * height),
    ]
...
def simulation():
    ...
    log(
        "garment-size=%.1fx%.1f avatar-span=%.1fx%.1f flat=%s start-height=%.1f top-pins=%s"
        % (garment_width, garment_height, plane_span, z_span, flat_avatar, scene.StartHeight, top_pins)
    )
    ...
    save(
        ...
        "simple %.0fx%.0f mm tunic before drape; orthographic front proves the initial garment orientation against the production mannequin" % (garment_width, garment_height),
    )
    ...
    log(
        "drape-bounds X=%.1f..%.1f Y=%.1f..%.1f Z=%.1f..%.1f facets=%d"
        % (drape.Mesh.BoundBox.XMin, drape.Mesh.BoundBox.XMax, drape.Mesh.BoundBox.YMin, drape.Mesh.BoundBox.YMax, drape.Mesh.BoundBox.ZMin, drape.Mesh.BoundBox.ZMax, drape.Mesh.CountFacets)
    )
    _make_preview_from_mesh(doc, drape, preview)
    preview.ViewObject.Visibility = False
    doc.recompute()
    ...
    for direction, method_name in directions:
        ...
        if method is None:
            raise RuntimeError("FreeCAD view method is unavailable: %s" % method_name)
        method(); _visual_fit(view)
        name = "cloth-simulation-draped-%s.png" % direction
        save(
            name,
        ...
        if direction == "front":
            save(
                ...
                "front alias retained for existing PR screenshot consumers",
            )
    _restore_tasks_after_visual_capture(task_dock)
    close_task()
...
try:
    ...
    if not Gui.getMainWindow().isVisible():
        raise RuntimeError("FreeCAD GUI main window is not visible")
