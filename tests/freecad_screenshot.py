...
def validate_task(panel, name, required):
    ...
    if not panel.form.isVisible(): panel.form.show(); panel.form.raise_(); events()
    texts = []
    for widget in [panel.form] + panel.form.findChildren(QtWidgets.QWidget):
        value = getattr(widget, "text", "")
        try: value = value() if callable(value) else value
...
def show_task(panel, name, required=()):
    ...
    Gui.Control.showDialog(panel); return validate_task(panel, name, required)
def close_task():
    if Gui.Control.activeDialog(): Gui.Control.closeDialog(); events()
def activate(name, toolbar, commands):
    ...
    window = Gui.getMainWindow()
    if window is None or not window.isVisible(): raise RuntimeError("FreeCAD main window is not visible")
...
def save(name, state, proof):
    ...
    if image.isNull() or (image.width(), image.height()) != (1280, 720): raise RuntimeError("invalid GUI capture for %s" % state)
...
def run_canonical_acceptance():
    for path, name, marker in (("tests/freecad_avatar_acceptance.py", "freecad_avatar_acceptance", "avatar-provider-acceptance"), ("tests/freecad_garment_e2e_smoke.py", "freecad_garment_e2e_smoke", "canonical-garment-e2e"), ("tests/freecad_simulation_quality_acceptance.py", "freecad_simulation_quality_acceptance", "simulation-quality-acceptance")):
...
def write_drape_metrics(panels, avatar):
    ...
    target_height = float(box.ZMax - box.ZMin); target_width = float(max(box.XMax - box.XMin, box.YMax - box.YMin)); records = []
    for panel in panels:
        vertices = _mesh_points(getattr(panel, "Mesh", None)); metrics = inspect_drape(vertices, avatar_vertices, target_height=target_height, target_width=target_width)
        record = {"panel": str(getattr(panel, "Label", getattr(panel, "Name", ""))), **summarize(metrics)}; records.append(record); log("drape-metrics=%s" % json.dumps(record, sort_keys=True))
    with open(METRICS, "w", encoding="utf-8") as handle: json.dump({"target_height": target_height, "target_width": target_width, "panels": records}, handle, indent=2, sort_keys=True)
def _make_tunic_sketch(doc, name, panel_width, garment_height, hem_width, neckline_ratio):
    ...
    points = [(0.00,0.00),(hem_width,0.00),(panel_width,0.82*garment_height),(0.86*panel_width,0.97*garment_height),(neckline_ratio*panel_width,garment_height),((1.0-neckline_ratio)*panel_width,garment_height),(0.14*panel_width,0.97*garment_height),(0.00,0.82*garment_height)]
    ...
    sketch.addConstraint([Sketcher.Constraint("Coincident",0,2,1,1),Sketcher.Constraint("Coincident",1,2,2,1),Sketcher.Constraint("Coincident",2,2,3,1),Sketcher.Constraint("Coincident",3,2,4,1),Sketcher.Constraint("Coincident",4,2,5,1),Sketcher.Constraint("Coincident",5,2,6,1),Sketcher.Constraint("Coincident",6,2,7,1),Sketcher.Constraint("Coincident",7,2,0,1)])
...
def _adopt_sketch(sketch, name, allowance, grainline):
    ...
    piece = create_pattern_piece_from_selected_sketch(name=name, allowance=allowance, grainline=grainline); piece.Label = name; doc = App.ActiveDocument; doc.recompute()
    if piece.Sketch is not sketch: raise RuntimeError("Cloth PatternPiece did not retain the selected native Sketcher source")
    return piece
def pattern_and_sewing():
    ...
    front_sketch, _front_outline = _make_tunic_sketch(doc,"VisualFront",520.0,720.0,600.0,0.64)
    back_sketch, _back_outline = _make_tunic_sketch(doc,"VisualBack",520.0,720.0,600.0,0.64)
    ...
    marker = doc.addObject("Part::Feature","GrainlineMarker"); marker.Shape = Part.makeLine(App.Vector(-400,90,1),App.Vector(-400,640,1)); front.ViewObject.Visibility=False; back.ViewObject.Visibility=False; front.Sketch.ViewObject.Visibility=True; back.Sketch.ViewObject.Visibility=True; doc.recompute()
    if front.Shape.isNull() or back.Shape.isNull(): raise RuntimeError("pattern fixture produced empty geometry from native sketches")
...
def style_mesh(obj, label):
    obj.Label = label
    try: obj.ViewObject.DisplayMode="Flat Lines"; obj.ViewObject.ShapeColor=(0.86,0.20,0.10); obj.ViewObject.LineColor=(0.20,0.02,0.01); obj.ViewObject.LineWidth=1.5
...
def simulation():
    ...
    if target is None: raise RuntimeError("visual fixture did not create DrapeTarget")
    box=avatar.Mesh.BoundBox; x_mid=(box.XMin+box.XMax)/2.0; y_span=box.YMax-box.YMin; z_span=box.ZMax-box.ZMin; chest=980.0; hip=1020.0; ease=55.0; panel_width=max(420.0,0.50*chest+ease); hem_width=max(450.0,0.50*hip+ease); shoulder_z=box.ZMin+0.76*z_span; hem_z=box.ZMin+0.40*z_span; garment_height=max(560.0,shoulder_z-hem_z); body_depth=max(120.0,min(260.0,y_span)); clearance=max(6.0,0.02*body_depth); front_y=box.YMin-clearance; back_y=box.YMax+clearance; rot=App.Rotation(App.Vector(1,0,0),90.0)
    ...
    front,front_outline=make_piece("VisualTunicFront",front_y,0.64); back,back_outline=make_piece("VisualTunicBack",back_y,0.68)
    for edge_a,edge_b,seam_id in ((1,7,"TunicRightSide"),(7,1,"TunicLeftSide"),(2,6,"TunicRightShoulder"),(6,2,"TunicLeftShoulder")): add_seam(doc,Seam(str(front.PieceId),edge_a,str(back.PieceId),edge_b,id=seam_id,alignment="uniform",stitch_group="TunicAssembly"))
    scene.StartHeight=0.0; scene.QualityPreset="Fast"; scene.ParticleDistance=22.0; scene.SolverIterations=6; scene.SolverSubsteps=1; scene.TimeStep=1.0/90.0; scene.GravityX=0.0; scene.GravityY=0.0; scene.GravityZ=-9810.0; scene.ClothPieces=[front,back]; refresh_drape_target(target); doc.recompute()
    def local_boundary(piece,outline):
        points=[(float(x),float(y)) for x,y in outline]; segments=[LineSegment("%s:edge:%d"%(piece.PieceId,i),points[i],points[(i+1)%len(points)]) for i in range(len(points))]; mesh=triangulate(ParametricPattern(segments)); h=max(y for _,y in points); pins=tuple(i for i in mesh.boundary_vertex_indices if float(mesh.vertices[i][1])>=0.86*h-1e-6 and (float(mesh.vertices[i][0])<=0.32*panel_width+1e-6 or float(mesh.vertices[i][0])>=0.68*panel_width-1e-6)); return mesh,pins
    fmesh,front_pins=local_boundary(front,front_outline); _,back_pins_local=local_boundary(back,back_outline); back_pins=tuple(len(fmesh.vertices)+i for i in back_pins_local); scene.PinSelection=[str(i) for i in front_pins+back_pins]; doc.recompute()
    ...
    if len(panels)!=2: raise RuntimeError("expected two drape panels, got %d"%len(panels))
    for panel,label in zip(panels,("Drape: Tunic Front","Drape: Tunic Back")): style_mesh(panel,label); panel.ViewObject.Visibility=True
    ...
    activate("ClothSimulationWorkbench","Cloth Simulation",["ClothSimulation_Edit"]); simulation_panel=SimulationQualityTaskPanel(scene); task_dock=show_task(simulation_panel,"Simulation Workbench arranged",("Preset","Particle distance","Density","Avatar skin offset","Simulation steps","Step","Run 30","Reset")); view=Gui.activeDocument().activeView(); view.setCameraType("Orthographic"); view.viewFront(); view.fitAll(); events(); task_dock.hide(); events(); save("cloth-simulation-arranged.png","Simulation Workbench arranged","vertical sewn tunic generated from native Sketcher pattern sources on production mannequin"); task_dock.show(); task_dock.raise_(); events()
    for batch in (10,10,10): simulation_panel.step(batch); doc.recompute(); events()
    if int(scene.Steps)!=30 or float(scene.SimulatedTime)<=0.0 or not bool(scene.FiniteState): raise RuntimeError("simulation did not reach a finite 30-step state")
    if any(panel.Mesh.CountFacets<=10 for panel in scene.DrapePanels): raise RuntimeError("draped tunic panel mesh is empty")
    write_drape_metrics(panels,avatar); bounds=[]
    for panel in scene.DrapePanels: b=panel.Mesh.BoundBox; bounds.append((b.XMin,b.XMax,b.YMin,b.YMax,b.ZMin,b.ZMax))
    ...
    for direction,method_name in (("front","viewFront"),("rear","viewRear"),("left","viewLeft"),("right","viewRight"),("top","viewTop"),("bottom","viewBottom")):
        ...
        if direction=="front": save("cloth-simulation-draped.png","Simulation Workbench draped front","legacy front screenshot alias; native Sketcher tunic source")
    task_dock.show(); task_dock.raise_(); events(); close_task(); App.closeDocument(doc.Name)
def main():
    log("script-start")
    if Gui.getMainWindow() is None or not Gui.getMainWindow().isVisible(): raise RuntimeError("FreeCAD GUI did not launch")
