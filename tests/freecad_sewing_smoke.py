...
def main():
    ...
    try:
        ...
        # Editing upstream pattern geometry must invalidate the semantic seam,
        # rather than silently following the same ordinal Edge0.
        piece_a.Width = 120
        doc.recompute()
        assert str(seam.EdgeAId) == f"{piece_a.PieceId}:edge:0"
        ...
        # Restore geometry and verify the captured semantic reference becomes valid again.
        piece_a.Width = 100
        doc.recompute()
        assert str(seam.Status) == "Valid"
        assert operation.Status == "Valid"
