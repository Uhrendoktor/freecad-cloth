...
def simulation():
    ...
    shoulder_z = box.ZMin + 0.76 * z_span; hem_z = box.ZMin + 0.40 * z_span; garment_height = max(560.0, shoulder_z - hem_z); clearance = 8.0; rot = App.Rotation(App.Vector(1,0,0), 90.0)
    ...
    front_anchors = (
        GarmentAnchor(str(front.PieceId), "shoulder-right", (0.86 * panel_width, 0.97 * garment_height, 0.0), "front"),
        GarmentAnchor(str(front.PieceId), "shoulder-left", (0.14 * panel_width, 0.97 * garment_height, 0.0), "front"),
    )
    back_anchors = (
        GarmentAnchor(str(back.PieceId), "shoulder-right", (0.86 * panel_width, 0.97 * garment_height, 0.0), "back"),
        GarmentAnchor(str(back.PieceId), "shoulder-left", (0.14 * panel_width, 0.97 * garment_height, 0.0), "back"),
    )
    ...
    exact_surface = collision_surface(target_source, float(getattr(target, "CollisionDeflection", 1.0)), float(getattr(target, "CollisionThickness", 0.0)))
    step_zero_positions = tuple(scene.Proxy._base_or_restore().backend.positions())
    step_zero_clearance = assert_minimum_surface_clearance(exact_surface, step_zero_positions, clearance)
