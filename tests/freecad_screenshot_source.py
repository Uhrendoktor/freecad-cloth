...
def simulation():
    ...
    shoulder_z = box.ZMin + 0.76 * z_span; hem_z = box.ZMin + 0.40 * z_span; garment_height = max(560.0, shoulder_z - hem_z); body_depth = max(120.0, min(260.0, y_span)); clearance = max(8.0, 0.025 * body_depth); front_y = box.YMax + clearance; back_y = box.YMin - clearance; rot = App.Rotation(App.Vector(1,0,0), 90.0)
    ...
    # Close the torso envelope with persistent side and shoulder seams; the neckline remains open.
    seam_records = []
    ...
    # Use the production fitting command to arrange the sewn garment around the target.
    from freecad_cloth.avatar.FittingCommands import create_fitting_scene, auto_arrange_garment, add_selected_pattern_pieces
    ...
    fitting_scene = auto_arrange_garment(clearance=clearance)
    scene.PinSelection = []
    release_front = front.Placement.multVec(front.Shape.BoundBox.Center)
    release_back = back.Placement.multVec(back.Shape.BoundBox.Center)
    if not (release_front.y > box.YMax and release_back.y < box.YMin):
        raise RuntimeError("auto-arranged tunic panels were not released on opposite sides of the target")
    if not (box.ZMin < release_front.z < box.ZMax and box.ZMin < release_back.z < box.ZMax):
        raise RuntimeError("auto-arranged tunic release height is outside the target")
