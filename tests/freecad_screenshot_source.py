...
def simulation():
    ...
    if any(float(result["distance_after"]) + 1e-6 < float(result["clearance"]) for result in snap_results):
        raise RuntimeError("target-aware tunic placement did not reach requested clearance")
    log("target-snap=passed pieces=%d clearances=%s" % (
        len(snap_results),
    ...
    save("cloth-simulation-step1.png", "Simulation Workbench tunic step 1", "first solver step after target-relative placement; finite state and non-empty panel evidence")
    log("step1=passed finite=%s panel-facets=%s" % (bool(scene.FiniteState), tuple(int(panel.Mesh.CountFacets) for panel in scene.DrapePanels)))
