...
def simulation():
    ...
    shoulder_z = box.ZMin + 0.76 * z_span; hem_z = box.ZMin + 0.40 * z_span; garment_height = max(560.0, shoulder_z - hem_z); body_depth = max(120.0, min(260.0, y_span)); clearance = max(20.0, 0.08 * body_depth); front_y = box.YMin - clearance; back_y = box.YMax + clearance
    front_angle, back_angle = wrapped_panel_angles(front_y, back_y, (box.YMin + box.YMax) / 2.0, 0.92 * garment_height)
    front_rot = App.Rotation(App.Vector(1,0,0), front_angle); back_rot = App.Rotation(App.Vector(1,0,0), back_angle)
    def make_piece(name, y, rotation, neckline_ratio, neckline_drop):
    ...
    front, front_outline = make_piece("VisualTunicFront", front_y, front_rot, 0.64, 0.10); back, back_outline = make_piece("VisualTunicBack", back_y, back_rot, 0.64, 0.07)
    ...
    # The panel shoulder edges are pre-arranged around the avatar by the
