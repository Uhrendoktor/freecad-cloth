...
def simulation():
    ...
    if any(
        _world_vertices(piece) and
        min(
            sum((point[i] - _surface_anchor(_world_collision_surface(target), point)[0][i]) * _surface_anchor(_world_collision_surface(target), point)[1][i] for i in range(3))
            for point in _world_vertices(piece)
        ) < clearance - 1e-6
        for piece in (front, back)
    ):
        raise RuntimeError("public target snap did not prove step-0 signed surface clearance")
    ...
    if not surface.vertices or not surface.triangles:
        raise RuntimeError("canonical tunic DrapeTarget has no authoritative collision triangles")
    proxy = scene.Proxy
    ...
    initial_clearance = min(
        min(
            sum((point[i] - _surface_anchor(surface, point)[0][i]) * _surface_anchor(surface, point)[1][i] for i in range(3))
            for point in _world_vertices(piece)
        )
        for piece in (front, back)
    )
    if float(initial_clearance) < float(clearance) - 1e-6:
        raise RuntimeError("canonical tunic step-0 target clearance is below configured separation: %.2f < %.2f" % (float(initial_clearance), float(clearance)))
    log("pin-mode=None solver-pins=0")
    log("target-snap=public-command home-placements-preserved")
    log("step0-target-signed-clearance-mm=%.2f required-mm=%.2f" % (float(initial_clearance), float(clearance)))
