...
def simulation():
    ...
    # The seed placement is deliberately target-independent. The fitting contract below
    # derives the mannequin-relative transform from authored garment anchors.
    ...
    front_result = target_aware_place_piece(front, target, front_anchors, clearance=clearance)
    back_result = target_aware_place_piece(back, target, back_anchors, clearance=clearance)
    log("target-aware-placement front=%s back=%s" % (front_result, back_result))
    ...
    if backend is None:
        raise RuntimeError("canonical tunic did not build a simulation backend after target-aware placement")
    surface = getattr(proxy, "collision_surface", None)
    if surface is None:
        raise RuntimeError("canonical tunic did not retain the authoritative DrapeTarget collision surface")
    step_zero_clearance = assert_minimum_surface_clearance(surface, tuple(backend.positions()), clearance)
