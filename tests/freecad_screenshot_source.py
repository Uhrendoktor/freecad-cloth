...
def simulation():
    ...
    box = target_source.Mesh.BoundBox; x_mid = (box.XMin + box.XMax) / 2.0; z_span = box.ZMax - box.ZMin
    ...
    doc.recompute()
    for piece in (front, back):
        placement_result = target_aware_place_piece(
            piece,
            target,
            anchors,
            clearance=clearance,
        ...
        log("target-aware-%s=%s" % (piece.PieceId, placement_result))
