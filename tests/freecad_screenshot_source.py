...
def simulation():
    ...
    box = avatar.Mesh.BoundBox; y_span = box.YMax - box.YMin
    from freecad_cloth.avatar.AvatarFitting import ArrangementPoint
    def arrangement_world(name):
        point = ArrangementPoint.from_string(next(v for v in avatar.ArrangementPoints if str(v).split("|", 1)[0] == name))
    ...
    log("pin-map auto=false explicit=[] collision-mode=%s" % __import__("os").environ.get("CLOTH_TISSU_COLLISION_MODE", "default"))
