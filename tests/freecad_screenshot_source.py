...
def _seam_coherence(panels, seam_records, proxy=None):
    ...
    for seam, piece_a, piece_b in seam_records:
        ...
        for piece, edge_id in ((piece_a, edge_a_id), (piece_b, edge_b_id)):
            semantic_ids = tuple(str(value) for value in getattr(getattr(piece, "Sketch", None), "SemanticEdgeIds", ()) or ())
...
def simulation():
    ...
    if len(front_edge_ids) < 8 or len(back_edge_ids) < 8 or any(
        not ids[index] for ids in (front_edge_ids, back_edge_ids) for index in (1, 2, 6, 7)
    ):
    ...
    for edge_a_id, edge_b_id, seam_id in seam_specs:
        ...
        if str(getattr(seam_obj, "EdgeAId", "")) != edge_a_id or str(getattr(seam_obj, "EdgeBId", "")) != edge_b_id:
            raise RuntimeError("canonical tunic seam %s did not retain semantic edge IDs" % seam_id)
