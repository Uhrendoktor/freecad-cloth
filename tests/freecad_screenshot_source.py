...
def simulation():
    ...
    from freecad_cloth.simulation.DrapeTarget import collision_surface, refresh_drape_target
    from freecad_cloth.avatar.AvatarArrangement import arrangement_point_map
    from freecad_cloth.pattern.PatternModel import Seam
    from freecad_cloth.pattern.PatternObjects import add_seam
    ...
    arrangement = arrangement_point_map(getattr(avatar, "ArrangementPoints", ()))
    missing = [name for name in ("chest", "waist", "hip", "shoulder_left", "shoulder_right") if name not in arrangement]
    if missing:
        raise RuntimeError("avatar arrangement metadata is missing: %s" % ",".join(missing))
    point = lambda name: tuple(float(value) for value in arrangement[name].split(","))
    shoulder_left = point("shoulder_left"); shoulder_right = point("shoulder_right"); chest_point = point("chest"); waist_point = point("waist"); hip_point = point("hip")
    box = avatar.Mesh.BoundBox; y_span = box.YMax - box.YMin; z_span = box.ZMax - box.ZMin
    chest = float(getattr(avatar, "Chest", 980.0)); hip = float(getattr(avatar, "Hip", 1020.0)); ease = 55.0; panel_width = max(420.0, 0.50 * chest + ease); hem_width = max(450.0, 0.50 * hip + ease)
    x_mid = 0.5 * (shoulder_left[0] + shoulder_right[0])
    shoulder_z = 0.5 * (shoulder_left[2] + shoulder_right[2])
    garment_height = max(560.0, 1.35 * max(1.0, shoulder_z - hip_point[2])); hem_z = shoulder_z - garment_height
    body_depth = max(120.0, min(260.0, y_span)); clearance = max(12.0, 0.05 * body_depth)
    target_surface = collision_surface(target.SourceObject, float(target.CollisionDeflection), float(target.CollisionThickness))
    chest_band = max(20.0, 0.12 * max(1.0, shoulder_z - waist_point[2]))
    chest_vertices = [vertex for vertex in target_surface.vertices if abs(float(vertex[2]) - float(chest_point[2])) <= chest_band]
    if not chest_vertices:
    ...
    proxy = scene.Proxy
    positions = tuple(proxy.backend.positions())
    if str(getattr(getattr(proxy, "backend", None), "name", "")) == "tissu" and tuple(getattr(proxy.backend, "_pin_indices", ())):
        raise RuntimeError("visual tunic simulation unexpectedly created solver pins")
    log("pin-policy automatic=%s explicit=%s" % (bool(scene.AutomaticPins), tuple(scene.PinSelection)))
    positions = tuple(proxy.backend.positions())
