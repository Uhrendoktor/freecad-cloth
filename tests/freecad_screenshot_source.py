...
def simulation():
    ...
    def authored_back_support_pins(piece, particle_indices, positions, seam_endpoints):
        targets = (
            (0.14 * panel_width, 0.97 * garment_height),
            (0.86 * panel_width, 0.97 * garment_height),
            (0.14 * panel_width, 0.82 * garment_height),
            (0.86 * panel_width, 0.82 * garment_height),
        )
        available = [index for index in particle_indices if int(index) not in seam_endpoints]
        result = []
        for local_x, local_y in targets:
            if not available:
                raise RuntimeError("visual tunic back-support candidates exhausted")
            target_point = piece.Placement.multVec(App.Vector(float(local_x), float(local_y), 0.0))
            index = min(
                available,
                key=lambda i: (
                    (positions[i][0] - target_point.x) ** 2
    ...
    seam_endpoints = {
        int(index)
        for stitch_pairs in getattr(proxy, "seam_stitch_pairs", {}).values()
        for pair in stitch_pairs
        for index in pair
    }
    back_support_pins = authored_back_support_pins(back, back_indices, positions, seam_endpoints)
    ...
    # position, making the zero-rest stitch constraint unsatisfiable. Keep the
    # authored front shoulder endpoints and add only non-seam back support
    # particles so the back panel cannot collapse through the avatar.
    pinned = tuple(front_pins) + tuple(back_support_pins)
    if any(
        int(a) in pinned and int(b) in pinned
    ...
    log(
        "pin-map authored front=%s back-support=%s back-endpoints-pinned=false"
        % (front_pins, tuple(back_support_pins))
    ); doc.recompute()
