...
def simulation():
    ...
    try:
        ...
        initial_clearance = None
    exact_step_zero = assert_minimum_surface_clearance(surface, tuple(backend.positions()), clearance)
    ...
    log("step0-target-vertex-clearance-mm=%.2f required-mm=%.2f authoritative-signed=%.2f" % (float(initial_clearance), float(clearance), float(exact_step_zero)))
