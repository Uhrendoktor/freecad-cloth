...
def test_project_math_has_no_freecad_dependency():
    from pathlib import Path

    source = (Path(__file__).resolve().parents[1] / "freecad_cloth" / "avatar" / "TargetPlacement.py").read_text(
        encoding="utf-8"
    )
...
if __name__ == "__main__":
    test_nearest_projection_reports_outward_normal_and_distance()
    test_signed_clearance_rejects_points_inside_target()
    test_nearest_projection_fails_closed_on_opposing_ambiguous_normals()
    test_average_point_is_deterministic()
    test_project_math_has_no_freecad_dependency()
    print("deterministic target projection tests passed")
