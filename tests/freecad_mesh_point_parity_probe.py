...
mesh_obj.Mesh = MeshPart.meshFromShape(Shape=shape, LinearDeflection=0.5, AngularDeflection=0.5)
doc.recompute()
...
for name, p in rays:
    hits = mesh_obj.Mesh.foraminate((p.x, p.y, p.z), (1.0, 0.0, 0.0), 3.141592653589793)
...
count = 0
for _ in range(1000):
    hits = mesh_obj.Mesh.foraminate((5.0, 6.0, 7.0), (1.0, 0.0, 0.0), 3.141592653589793)
    count += len(hits)
