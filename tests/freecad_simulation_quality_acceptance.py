...
def run_acceptance():
    ...
    try:
        ...
        doc.recompute()
        _activate("ClothSimulationWorkbench", ["ClothDrape_CreateTarget", "ClothDrape_RefreshTarget", "ClothSimulation_Edit"])
        Gui.Selection.clearSelection()
        ...
        doc.recompute()
        fast = {"particle": float(scene.ParticleDistance), "iterations": int(scene.SolverIterations), "substeps": int(scene.SolverSubsteps), "density": float(scene.FabricDensity), "thickness": float(scene.FabricThickness)}
        final = preset("Final")
