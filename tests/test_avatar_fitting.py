...
class AvatarFittingTests(unittest.TestCase):
    ...
    def test_target_snap_command_requires_explicit_single_target(self):
        from pathlib import Path
        ...
        self.assertIn('PiecePlacement.from_string', source)
        self.assertIn('HomePlacements', (Path(__file__).resolve().parents[1] / "freecad_cloth" / "avatar" / "FittingCommands.py").read_text(encoding="utf-8"))
