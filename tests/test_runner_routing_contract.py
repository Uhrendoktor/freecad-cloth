...
def test_public_pull_requests_are_hosted_only():
...
def test_trusted_jobs_route_through_router():
    source = WORKFLOW.read_text(encoding="utf-8")
...
def test_pr_checkout_uses_head_sha():
    source = WORKFLOW.read_text(encoding="utf-8")
    assert "github.event.pull_request.head.sha" in source
    assert "persist-credentials: false" in source

def test_stale_run_cleanup_uses_normal_pull_request_event():
    source = WORKFLOW.read_text(encoding="utf-8")
