"""Fast deterministic drape inspection harness.

Inspects the real production backend without adding a second pin/stitch layer.
The render uses a transparent avatar, current stitch lines, active pin markers,
and checkpoint metrics so bad topology is distinguishable from a lucky outline.
"""
...
def min_distance(source, target):
    ...
    for a in source:
        for b in target:
            best = min(best, sum((float(a[i]) - float(b[i])) ** 2 for i in range(3)))
...
def checkpoint_metrics(backend, pins, initial_pins, target_vertices, triangles, step):
    ...
    pin_drifts = [
        math.dist(positions[int(i)], p0)
        for i, p0 in zip(pins, initial_pins)
    ]
    ...
    result = {
        ...
        "minimum_vertex_to_target_mm": min_distance(positions, target_vertices),
...
def run():
    ...
    try:
        for target_step in STEPS:
            ...
            update_debug_geometry(doc, seam_obj, pin_obj, backend, pins)
            events()
            checkpoints.append(checkpoint_metrics(
