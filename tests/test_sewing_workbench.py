...
from SeamGeometry import correspondence, polyline_length, sample_polyline
...
def test_curved_like_edge_samples_use_arc_length():
    edge = [(0, 0), (3, 4), (6, 4)]
    assert polyline_length(edge) == 8.0
    assert sample_polyline(edge, 0.0, 1.0, 3) == ((0.0, 0.0), (3.0, 4.0), (6.0, 4.0))
    p = SimpleNamespace(Width=1, Height=1, SewingOutline=repr([(0, 0), (1, 0), (1, 1)]), SewingEdgeSamples=repr({"0": edge}))
    assert _edge_length(p, 0) == 8.0


def test_correspondence_reverses_only_second_edge():
    pairs = correspondence([(0, 0), (10, 0)], [(0, 5), (10, 5)], 0, 1, 0, 1, 3, True)
    assert pairs[0] == ((0.0, 0.0), (10.0, 5.0))
    assert pairs[-1] == ((10.0, 0.0), (0.0, 5.0))


def test_canonical_seam_contract_and_graph_adapter():
    a = SimpleNamespace(id="A", name="A", outline=[(0, 0), (10, 0), (10, 5)])
    b = SimpleNamespace(id="B", name="B", outline=[(0, 0), (10, 0), (10, 5)])
    from PatternModel import PatternPiece
    graph = SeamGraph()
    graph.add_piece(PatternPiece("A", a.outline, id="A"))
    graph.add_piece(PatternPiece("B", b.outline, id="B"))
    seam = Seam("A", 0, "B", 0, id="s1", reversed_b=True, alignment="uniform", stitch_group="g")
    graph.add_seam(seam)
    assert graph.seams["s1"].seam is seam
    assert graph.seams["s1"].alignment == "uniform"
    graph.validate()
...
def test_proxy_validation_and_reversal():
    ...
    try:
        seam = SimpleNamespace(EdgeA=0, StartA=0, EndA=1, EdgeB=0, StartB=0, EndB=1, ReversedB=True, Alignment="endpoints", StitchGroup="g")
        ...
        obj = SimpleNamespace(Seam=seam, PieceA=a, PieceB=b, Tolerance=.5, Stitches=8, Status="Incomplete", LengthA=0, LengthB=0, LengthDifference=0, StitchCount=0, StitchPoints=[], Shape=None, ReversedB=False, Alignment="endpoints", StitchGroup="g", AssemblyPlacementB=None)
...
if __name__ == "__main__":
    test_rectangular_seam_lengths(); test_polygon_seam_length_uses_stored_outline(); test_curved_like_edge_samples_use_arc_length(); test_correspondence_reverses_only_second_edge(); test_canonical_seam_contract_and_graph_adapter(); test_boundary_sampling_honors_normalized_range(); test_proxy_validation_and_reversal(); print("sewing tests passed")
