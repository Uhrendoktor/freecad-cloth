...
def test_function_command_uses_callable_metadata_and_returns_result():
    ...
    assert command.GetResources() == {"MenuText": "Example Command", "ToolTip": "Example command tooltip."}


def test_function_command_accepts_explicit_tooltip():
    def example_command(): return None
    assert FunctionCommand(example_command, "Explicit tooltip").GetResources()["ToolTip"] == "Explicit tooltip"


def test_register_commands_is_headless_safe_and_registers_each_callable():
    registered = {}
    class FakeGui:
        def addCommand(self, name, command): registered[name] = command
