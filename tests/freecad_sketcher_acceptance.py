...
def run_acceptance():
    ...
    try:
        ...
        elif seam_edit_path == "commit":
            dialog = Gui.Control.activeDialog()
            _record("seam-edit-commit-dialog-before")
            if dialog is None or not callable(getattr(dialog, "accept", None)):
                raise RuntimeError("staged seam creation dialog is not active/committable")
            dialog.accept()
            _events()
            _record("seam-edit-commit-dialog-after")
            _record("seam-edit-commit-before-clear-selection")
            Gui.Selection.clearSelection()
            _record("seam-edit-commit-after-clear-selection")
            _record("seam-edit-commit-before-select-piece")
            Gui.Selection.addSelection(curved)
            _record("seam-edit-commit-after-select-piece")
            _record("seam-edit-commit-before-set-edit")
            Gui.activeDocument().setEdit(curved_sketch.Name)
            _record("seam-edit-commit-after-set-edit")
            _record("seam-edit-commit-before-clear-selection-2")
            Gui.Selection.clearSelection()
            _record("seam-edit-commit-after-clear-selection-2")
            _record("seam-edit-commit-before-select-sketch-edge")
            Gui.Selection.addSelection(curved_sketch, "Edge%d" % (edge_index + 1))
            _record("seam-edit-commit-after-select-sketch-edge")
