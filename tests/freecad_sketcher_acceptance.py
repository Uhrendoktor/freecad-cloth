...
def run_acceptance():
    ...
    try:
        ...
        elif seam_edit_path == "minimal":
            _record("seam-edit-minimal-before-clear-selection")
            Gui.Selection.clearSelection()
            _record("seam-edit-minimal-after-clear-selection")
            _record("seam-edit-minimal-before-select-piece")
            Gui.Selection.addSelection(curved)
            _record("seam-edit-minimal-after-select-piece")
            _record("seam-edit-minimal-before-selection-events")
            _events()
            _record("seam-edit-minimal-after-selection-events")
            _record("seam-edit-minimal-before-set-edit")
            Gui.activeDocument().setEdit(curved_sketch.Name)
            _record("seam-edit-minimal-after-set-edit")
            _record("seam-edit-minimal-before-clear-selection-2")
            Gui.Selection.clearSelection()
            _record("seam-edit-minimal-after-clear-selection-2")
            _record("seam-edit-minimal-before-select-sketch-edge")
            Gui.Selection.addSelection(curved_sketch, "Edge%d" % (edge_index + 1))
            _record("seam-edit-minimal-after-select-sketch-edge")
