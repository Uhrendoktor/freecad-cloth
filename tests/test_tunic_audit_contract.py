...
def test_tunic_audit_resolves_seam_diagnostics_by_semantic_id():
    ...
    assert 'edge_index = {str(boundary.id): index for index, boundary in enumerate(piece_ir.boundaries)}' in audit
