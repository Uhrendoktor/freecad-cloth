...
def test_canonical_tunic_uses_target_relative_arrangement_and_no_solver_pins():
    source = (ROOT / "tests" / "freecad_screenshot_source.py").read_text(encoding="utf-8")
    assert "arrange_pattern_pieces_to_target" in source
    assert 'scene.PinMode = "None"' in source
    assert "scene.PinSelection = []" in source
    assert "authored_shoulder_pins" not in source
    assert "tunic-pin-mode=None" in source
