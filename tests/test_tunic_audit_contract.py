...
def test_canonical_tunic_visual_gate_is_fail_closed():
    audit = (ROOT / "tests" / "freecad_tunic_audit.py").read_text(encoding="utf-8")
    assert 'classification.state in {"detached-candidate", "edge-on-candidate"}' in audit
    assert '"collapsed-candidate" in diagnostics' in audit
    assert "canonical tunic visual geometry is not structurally plausible" in audit
