...
def main():
    ...
    try:
        ...
        Gui.updateGui()
        trace("stage=gui-initialized elapsed_ms=%.3f" % (1000.0 * (time.perf_counter() - run_started)))
        from freecad_cloth.simulation.RealtimePreview import _prepare, _select_preview_backend
        from freecad_cloth.simulation.SimulationQualityRuntimeV2 import create_quality_simulation_scene
        trace("stage=benchmark-imports-ready elapsed_ms=%.3f" % (1000.0 * (time.perf_counter() - run_started)))
