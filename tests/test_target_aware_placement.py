...
def test_garment_anchor_contract_contains_shoulder_side_and_waist():
    from freecad_cloth.avatar.AvatarFitting import GarmentAnchor

    anchors = (
        GarmentAnchor("front", "shoulder_left", (1, 2, 3), "front"),
        GarmentAnchor("front", "shoulder_right", (4, 2, 3), "front"),
        GarmentAnchor("front", "side_left", (0, 5, 3), "front"),
        GarmentAnchor("front", "side_right", (5, 5, 3), "front"),
        GarmentAnchor("front", "waist", (2.5, 5, 3), "front"),
    )
    assert [anchor.name for anchor in anchors] == [
        "shoulder_left", "shoulder_right", "side_left", "side_right", "waist"
    ]
    assert [GarmentAnchor.from_string(anchor.to_string()).name for anchor in anchors] == [
        anchor.name for anchor in anchors
    ]
...
def test_target_aware_fitting_command_is_on_fitting_surface():
    ...
    namespace = {}
    exec("def target_aware_arrange_selected(): pass", namespace)
    assert '"ClothFitting_TargetAwareArrange": target_aware_arrange_selected' in commands
