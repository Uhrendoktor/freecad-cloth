...
def test_target_aware_arrangement_command_is_public():
    from pathlib import Path
    root = Path(__file__).resolve().parents[1]
    fitting = (root / "freecad_cloth" / "avatar" / "FittingCommands.py").read_text(encoding="utf-8")
    assert "def arrange_garment_on_avatar(" in fitting
    assert '"ClothFitting_ArrangeGarmentOnAvatar"' in fitting
