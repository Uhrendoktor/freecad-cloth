...
def _edge_points(piece,edge,start,end,z=.2):
    ...
    p0=App.Vector(a[0]+(b[0]-a[0])*start,a[1]+(b[1]-a[1])*start,z);p1=App.Vector(a[0]+(b[0]-a[0])*end,a[1]+(b[1]-a[1])*end,z);placement=getattr(piece,"Placement",None)
...
class SewingOperationProxy:
    ...
    def execute(self,obj):
        ...
        if seam is None or piece_a is None or piece_b is None:obj.Status="Incomplete";obj.LengthA=obj.LengthB=obj.LengthDifference=0.;obj.StitchCount=0;obj.Shape=Part.Shape();return
...
def add_sewing_operation(doc,seam,piece_a,piece_b,name="SewingOperation"):
    obj=doc.addObject("Part::FeaturePython",name);obj.Label=name;obj.addProperty("App::PropertyString","SewingType","Sewing").SewingType="SewingOperation";obj.addProperty("App::PropertyLink","Seam","Sewing").Seam=seam;obj.addProperty("App::PropertyLink","PieceA","Sewing").PieceA=piece_a;obj.addProperty("App::PropertyLink","PieceB","Sewing").PieceB=piece_b;obj.addProperty("App::PropertyString","StitchGroup","Sewing").StitchGroup=str(getattr(seam,"StitchGroup","") or getattr(seam,"SeamId","") or getattr(seam,"Name",""));obj.addProperty("App::PropertyEnumeration","Alignment","Sewing").Alignment=["endpoints","uniform"];obj.Alignment="endpoints";obj.addProperty("App::PropertyBool","ReversedB","Sewing").ReversedB=bool(getattr(seam,"ReversedB",False));obj.addProperty("App::PropertyPlacement","AssemblyPlacementB","Assembly").AssemblyPlacementB=piece_b.Placement if hasattr(piece_b,"Placement") else None;obj.addProperty("App::PropertyLength","Tolerance","Validation").Tolerance=.5;obj.addProperty("App::PropertyInteger","Stitches","Stitching").Stitches=8;obj.addProperty("App::PropertyLength","LengthA","Validation").LengthA=0;obj.addProperty("App::PropertyLength","LengthB","Validation").LengthB=0;obj.addProperty("App::PropertyLength","LengthDifference","Validation").LengthDifference=0;obj.addProperty("App::PropertyInteger","StitchCount","Stitching").StitchCount=8;obj.addProperty("App::PropertyString","Status","Validation").Status="Incomplete";obj.Proxy=SewingOperationProxy();obj.Proxy.execute(obj);return obj
