...
def _seam_length(piece, seam, prefix):
    ...
    if not 0.0 <= start <= 1.0 or not 0.0 <= end <= 1.0: raise ValueError("seam parameters must be between 0 and 1")
...
def _edge_points(piece, edge, start, end, z=0.2):
    ...
    corners = {0: ((0.0, 0.0), (width, 0.0)), 1: ((width, 0.0), (width, height)), 2: ((width, height), (0.0, height)), 3: ((0.0, height), (0.0, 0.0))}
    try: a, b = corners[int(edge)]
    except KeyError: raise ValueError("seam edge must be between 0 and 3")
    local_a = App.Vector(a[0] + (b[0]-a[0])*start, a[1] + (b[1]-a[1])*start, z)
    local_b = App.Vector(a[0] + (b[0]-a[0])*end, a[1] + (b[1]-a[1])*end, z)
    placement = getattr(piece, "Placement", None)
    if placement is not None:
        local_a, local_b = placement.multVec(local_a), placement.multVec(local_b)
...
class SewingOperationProxy:
    ...
    def execute(self, obj):
        ...
        length_a, length_b = _seam_length(piece_a, seam, "A"), _seam_length(piece_b, seam, "B")
        obj.LengthA, obj.LengthB, obj.LengthDifference = length_a, length_b, abs(length_a-length_b)
        obj.StitchCount = max(2, int(obj.Stitches)); obj.Status = "Valid" if obj.LengthDifference <= max(0.0, float(obj.Tolerance)) else "Length mismatch"
        sa, ea, sb, eb = float(seam.StartA), float(seam.EndA), float(seam.StartB), float(seam.EndB)
        if bool(seam.ReversedB): sb, eb = 1.0-eb, 1.0-sb
        a0, a1 = _edge_points(piece_a, seam.EdgeA, sa, ea); b0, b1 = _edge_points(piece_b, seam.EdgeB, sb, eb)
