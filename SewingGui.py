...
class SewingTaskPanel:
    ...
    def __init__(self,obj):
        ...
        self.seam_info=QtWidgets.QLabel("%s: %s ↔ %s"%(seam_id or "Unassigned seam",piece_a or "?",piece_b or "?")); self.seam_info.setWordWrap(True); self.seam_info.setAccessibleName("Sewing seam summary"); layout.addRow("Seam",self.seam_info)
        self.alignment=QtWidgets.QComboBox(); self.alignment.addItems(["endpoints","uniform"]); current_alignment=str(getattr(self.seam,"Alignment",getattr(obj,"Alignment","endpoints"))); self.alignment.setCurrentIndex(max(0,self.alignment.findText(current_alignment))); self.alignment.setToolTip("Choose how stitch positions correspond along the paired seam edges"); layout.addRow("Alignment",self.alignment)
        self.reversed_b=QtWidgets.QCheckBox("Reverse B correspondence"); self.reversed_b.setChecked(bool(getattr(self.seam,"ReversedB",getattr(obj,"ReversedB",False)))); self.reversed_b.setToolTip("Reverse the direction used to match stitch positions on the B-side seam"); layout.addRow("Orientation",self.reversed_b)
        ...
        self.tolerance=QtWidgets.QDoubleSpinBox(); self.tolerance.setRange(0,1000); self.tolerance.setDecimals(2); self.tolerance.setSuffix(" mm"); self.tolerance.setToolTip("Maximum allowed seam-length discrepancy during validation"); self.tolerance.setValue(float(obj.Tolerance)); layout.addRow("Validation tolerance",self.tolerance)
        self.stitches=QtWidgets.QSpinBox(); self.stitches.setRange(2,10000); self.stitches.setValue(max(2,int(obj.Stitches))); self.stitches.setToolTip("Number of stitch samples used for the sewing correspondence"); layout.addRow("Stitch samples",self.stitches)
        ...
        # Repair is an explicit secondary/recovery action; OK remains the
        # native primary task-panel action and Cancel remains the standard
        # destructive/rollback action supplied by FreeCAD.
        self.repair_button=QtWidgets.QPushButton("Repair Seam Correspondence"); self.repair_button.setToolTip("Repair reversible or invalid-range correspondence without hiding physical length mismatch"); self.repair_button.setAccessibleName("Repair seam correspondence"); self.repair_button.setAccessibleDescription("Secondary recovery action for reversible or invalid-range seam correspondence"); self.repair_button.setAutoDefault(False); self.repair_button.setDefault(False); self.repair_button.setProperty("actionRole", "secondary"); self.repair_button.clicked.connect(self.repair); layout.addRow("Recovery",self.repair_button)
