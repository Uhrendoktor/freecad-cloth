...
class SimulationQualityTaskPanel:
    ...
    def __init__(self, scene=None):
        ...
        self.density = self._double(1.0, 2000.0, 150.0, 1); self.thickness = self._double(0.01, 10.0, 0.5, 2)
        self.stretch = self._double(0.0, 1.0, 0.02, 4); self.shear = self._double(0.0, 1.0, 0.02, 4)
        self.bend = self._double(0.0, 1.0, 0.01, 4); self.friction = self._double(0.0, 1.0, 0.5, 3)
        for label, widget in (("Density (g/m²)", self.density), ("Thickness (mm)", self.thickness),
                              ("Stretch", self.stretch), ("Shear", self.shear), ("Bend", self.bend),
        ...
        cform.addRow("Avatar skin offset (mm)", self.skin_offset)
        cform.addRow("Fallback sphere radius (mm)", self.collision_radius)
        root.addWidget(collision)
        target = QtWidgets.QGroupBox("Drape target")
        tform = QtWidgets.QVBoxLayout(target)
        ...
        tform.addWidget(self.target_status); tform.addWidget(self.refresh_target_button)
        ...
        self.refresh_target_button.clicked.connect(self.refresh_target)
        self._load()
    ...
    def _load(self):
        ...
        ensure_quality_properties(self.scene)
        widgets = [self.quality, self.particle_distance, self.iterations, self.substeps, self.density, self.thickness,
                   self.stretch, self.shear, self.bend, self.friction, self.skin_offset, self.collision_radius, self.steps]
    ...
    def step(self, count):
        ...
        if status["stale"]:
            self._refresh("Simulation blocked: %s" % status["message"])
            return False
    ...
    def refresh_target(self):
        ...
        try:
            ...
            refresh_drape_target(getattr(self.scene, "DrapeTarget", None))
            self.scene.Document.recompute()
        except (AttributeError, TypeError, ValueError) as exc:
            self._refresh("Drape target refresh failed: %s" % exc)
            return False
    ...
    def _refresh_target_state(self):
        if self.scene is None:
            self.target_status.setText("Target: unavailable")
            self.step_button.setEnabled(False); self.run_button.setEnabled(False); self.refresh_target_button.setEnabled(False)
            return
        try:
            from DrapeTarget import target_status
            info = target_status(getattr(self.scene, "DrapeTarget", None))
        except (ImportError, AttributeError, TypeError, ValueError) as exc:
            info = {"state": "invalid", "message": str(exc), "stale": True, "reason": "inspection failed"}
        self.target_status.setText("Target: %s — %s" % (info["state"], info["message"]))
        ready = not info["stale"]
        self.step_button.setEnabled(ready); self.run_button.setEnabled(ready)
        self.refresh_target_button.setEnabled(info["state"] in ("stale", "unbuilt", "ready"))
