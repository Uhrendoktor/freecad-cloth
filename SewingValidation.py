...
def seam_diagnostic(seam,piece_map=None):
    ...
    if piece_map:
        try:
            ...
            if result["length_a"]<=0.0 or result["length_b"]<=0.0:
                result["status"]="Invalid"; result["message"]="Unable to resolve seam geometry: %s"%exc; return result
...
def validate_sewing_graph(doc,tolerance=0.5):
    pieces=_pattern_pieces(doc); piece_map={str(getattr(p,"PieceId","")):p for p in pieces}; seams=_seams(doc); networks=_networks(doc)
    diagnostics=[seam_diagnostic(seam,piece_map) for seam in seams]; invalid=[]; connected=set()
