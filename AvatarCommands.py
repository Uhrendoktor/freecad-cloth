...
def _set_prop(obj, kind, name, group, value):
    if not hasattr(obj, name): obj.addProperty(kind, name, group)
...
def _limb(base, end, radius):
    ...
    a, b = App.Vector(*base), App.Vector(*end); direction = b.sub(a)
    if direction.Length < 1e-6: return Part.makeSphere(radius, a)
...
def _rebuild(obj):
    ...
    obj.Shape=shape; obj.ParametersJSON=p.to_json(); obj.AvatarStatus="Valid"; obj.Landmarks=["neck|0,0,%.3f"%neck_z,"chest|0,0,%.3f"%chest_z,"waist|0,0,%.3f"%waist_z,"hip|0,0,%.3f"%pelvis_z,"shoulder_left|%.3f,0,%.3f"%(-shoulder_half,arm_z),"shoulder_right|%.3f,0,%.3f"%(shoulder_half,arm_z),"knee_left|%.3f,0,%.3f"%(-leg_x,knee_z),"knee_right|%.3f,0,%.3f"%(leg_x,knee_z)]
    obj.Document.recompute(); return obj

def create_avatar():
    import FreeCAD as App
    doc=App.ActiveDocument or App.newDocument("ClothSewing"); obj=_avatar(doc)
    if obj is None:
...
def set_avatar_pose(pose):
    ...
    obj=_avatar(doc) or create_avatar(); Pose(str(pose)).validate(); obj.PosePreset=str(pose); return _rebuild(obj)

def set_avatar_skin_offset(offset):
    import FreeCAD as App
    doc=App.ActiveDocument
    if doc is None: raise ValueError("open a document before changing avatar offset")
    obj=_avatar(doc) or create_avatar(); obj.SkinOffset=float(offset); return _rebuild(obj)
