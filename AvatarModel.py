...
class AvatarParameters:
    ...
    def from_json(cls, payload):
        ...
        p = data.get("pose", {})
        pose = Pose(str(p.get("preset", "standing")), float(p.get("left_arm_angle", 12)),
                    float(p.get("right_arm_angle", 12)), float(p.get("left_elbow_angle", 0)),
                    float(p.get("right_elbow_angle", 0)))
        return cls(data.get("measurements", {}), float(data.get("skin_offset", 3)), pose,
...
def _capsule(vertices, triangles, a, b, radius, segments=16):
    ...
    # The current representation is intentionally solver-neutral.  A compact
    # ellipsoid gives stable collision geometry; the service API permits a more
    # anatomical generator to replace it later without changing callers.
...
def generate_mesh(params):
    ...
    shoulder_z = min(m["height"] - 180, chest_z + 150)
    neck_z = shoulder_z + 100
    rings = [_ring(vertices, pelvis_z-100, hip_w, hip_d),
             _ring(vertices, pelvis_z, hip_w*1.02, hip_d*1.02),
    ...
    for side in (-1, 1):
        ...
        ex = side * (shoulder_half + 125)
        wx = side * (shoulder_half + 245)
    ...
    landmarks = {
        ...
        "underbust": (0, 0, chest_z-75), "waist": (0, 0, waist_z),
        "high_hip": (0, 0, waist_z-70), "hip": (0, 0, pelvis_z),
        "crotch": (0, 0, pelvis_z-65),
        "shoulder_left": (-shoulder_half, 0, arm_z),
        "shoulder_right": (shoulder_half, 0, arm_z),
        "knee_left": (-hip_offset, 0, knee_z), "knee_right": (hip_offset, 0, knee_z),
        "ankle_left": (-hip_offset, 0, ankle_z), "ankle_right": (hip_offset, 0, ankle_z),
        "wrist_left": (-shoulder_half-245, 0, arm_z-55),
        "wrist_right": (shoulder_half+245, 0, arm_z-55),
    }
