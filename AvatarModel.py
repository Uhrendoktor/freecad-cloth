...
class AvatarParameters:
    ...
    def to_json(self):
        self.validate()
        return json.dumps({
            "schema_version": self.schema_version, "units": "mm",
            "measurements": dict(sorted(self.measurements.items())),
            "skin_offset": float(self.skin_offset),
            "pose": {"preset": self.pose.preset, "left_arm_angle": self.pose.left_arm_angle,
                     "right_arm_angle": self.pose.right_arm_angle, "left_elbow_angle": self.pose.left_elbow_angle,
...
def generate_mesh(params):
    ...
    rings = [_ring(vertices, pelvis_z-100, hip_w, hip_d), _ring(vertices, pelvis_z, hip_w*1.02, hip_d*1.02),
             _ring(vertices, waist_z, waist_w, waist_d), _ring(vertices, chest_z, chest_w, chest_d),
    ...
    landmarks = {"neck": (0,0,neck_z), "chest": (0,0,chest_z), "underbust": (0,0,chest_z-75), "waist": (0,0,waist_z),
        "high_hip": (0,0,waist_z-70), "hip": (0,0,pelvis_z), "crotch": (0,0,pelvis_z-65),
        "shoulder_left": (-shoulder_half,0,arm_z), "shoulder_right": (shoulder_half,0,arm_z),
        ...
        "wrist_left": (-shoulder_half-245,0,arm_z-55), "wrist_right": (shoulder_half+245,0,arm_z-55)}
