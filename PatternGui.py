...
class PatternPieceTaskPanel:
    def __init__(self, obj=None):
        ...
        if obj:
            ...
            self._original = {"Label": obj.Label, "GeometryMode": str(getattr(obj, "GeometryMode", "Rectangle")), "Width": float(obj.Width), "Height": float(obj.Height), "SeamAllowance": float(obj.SeamAllowance), "GrainlineAngle": float(obj.GrainlineAngle)}
        else:
    ...
    def _mode_changed(self, mode):
        custom = mode == "Custom"
        self.width.setEnabled(not custom); self.height.setEnabled(not custom)

    def _validate(self):
        name = self.name.text().strip()
        if not name: raise ValueError("pattern piece name must not be empty")
        if self.mode.currentText() == "Rectangle" and (self.width.value() <= 0 or self.height.value() <= 0): raise ValueError("pattern piece dimensions must be positive")
        if self.allowance.value() < 0: raise ValueError("seam allowance cannot be negative")

    def _apply(self):
        self._validate(); mode = self.mode.currentText()
        if self.obj is None:
        ...
        if mode == "Rectangle": self.obj.GeometryMode = "Rectangle"; self.obj.Width = self.width.value(); self.obj.Height = self.height.value()
        else: self.obj.GeometryMode = "Custom"
        self.obj.SeamAllowance = self.allowance.value(); self.obj.GrainlineAngle = self.grain.value(); self.obj.Label = self.name.text().strip() or self.obj.Label
        self.App.ActiveDocument.recompute(); self.Gui.activeDocument().activeView().viewTop(); self.Gui.activeDocument().activeView().fitAll()

    def _restore(self):
        if self.obj is None or self._original is None: return
        for key, value in self._original.items(): setattr(self.obj, key, value)
...
class PatternDraftingTaskPanel:
    ...
    def __init__(self, obj):
        ...
        self._original = {"GeometryMode": str(getattr(obj, "GeometryMode", "Custom")), "DraftingBoundary": str(getattr(obj, "DraftingBoundary", "")), "Width": float(obj.Width), "Height": float(obj.Height)}
        self.move_point, self.add_point, self.remove_point = move_point, add_point, remove_point
        self.seam_allowance_preview = seam_allowance_preview; self.selected = 0
        self.form = QtWidgets.QWidget(); self.form.setWindowTitle("Cloth Pattern — 2D Drafting")
        outer = QtWidgets.QVBoxLayout(self.form); self.canvas = QtWidgets.QGraphicsView(); self.scene = QtWidgets.QGraphicsScene(self.canvas)
        self.canvas.setScene(self.scene); self.canvas.setMinimumSize(500, 360); outer.addWidget(self.canvas); self.scene.selectionChanged.connect(self._selection_changed)
        controls = QtWidgets.QHBoxLayout()
        buttons = (("Nudge left", "ClothPattern_NudgeLeft.svg", -5, 0), ("Nudge right", "ClothPattern_NudgeRight.svg", 5, 0), ("Nudge up", "ClothPattern_NudgeUp.svg", 0, 5), ("Nudge down", "ClothPattern_NudgeDown.svg", 0, -5))
        for text, icon_name, dx, dy in buttons:
            button = QtWidgets.QPushButton(text); button.setIcon(QtGui.QIcon(icon_name)); button.setToolTip(text)
            button.clicked.connect(lambda _=False, x=dx, y=dy: self.nudge(x, y)); controls.addWidget(button)
        add_button = QtWidgets.QPushButton("Add point"); add_button.setIcon(QtGui.QIcon("ClothPattern_AddPoint.svg")); add_button.setToolTip("Add point"); add_button.clicked.connect(self.add_selected_point); controls.addWidget(add_button)
        remove_button = QtWidgets.QPushButton("Remove point"); remove_button.setIcon(QtGui.QIcon("ClothPattern_RemovePoint.svg")); remove_button.setToolTip("Remove point"); remove_button.clicked.connect(self.remove_selected_point); controls.addWidget(remove_button)
        self.point_label = QtWidgets.QLabel("Point 0"); controls.addWidget(self.point_label); outer.addLayout(controls)
        outer.addWidget(QtWidgets.QLabel("Select a boundary point, then move/add/remove it. The polygon is stored as the authoritative sewing boundary.")); self._redraw(QtGui, QtCore)
    ...
    def _redraw(self, QtGui, QtCore):
        ...
        poly = [cv(p) for p in pts] + [cv(pts[0])]; self.scene.addPolygon(QtGui.QPolygonF(poly), QtGui.QPen(QtGui.QColor("#204a87"), 2))
        ...
        for i in range(len(pts)):
            a, b = cv(pts[i]), cv(pts[(i + 1) % len(pts)]); mid = (a + b) / 2; self.scene.addText("S%d" % i).setPos(mid.x(), mid.y())
    ...
    def _persist(self):
        ...
        self.obj.GeometryMode = "Custom"; self.obj.DraftingBoundary = serialize_points(self.points); x0, y0, x1, y1 = bounds(self.points); self.obj.Width = max(.001, x1 - x0); self.obj.Height = max(.001, y1 - y0); self.App.ActiveDocument.recompute(); _, _, _, QtGui, QtCore = _gui_modules(); self._redraw(QtGui, QtCore)

    def nudge(self, dx, dy):
        p = self.points[self.selected]; self.points = list(self.move_point(self.points, self.selected, p[0] + dx, p[1] + dy)); self._persist()

    def add_selected_point(self):
        a = self.points[self.selected]; b = self.points[(self.selected + 1) % len(self.points)]; point = ((a[0] + b[0]) / 2.0, (a[1] + b[1]) / 2.0); self.points = list(self.add_point(self.points, point[0], point[1], self.selected + 1)); self.selected += 1; self._persist()
