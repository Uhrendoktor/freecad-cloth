...
class PatternPieceTaskPanel:
    def __init__(self, obj=None):
        ...
        self.mode = QtWidgets.QComboBox(); self.mode.addItems(["Rectangle", "Custom"])
        ...
        if obj:
            self.name.setText(obj.Label); self.mode.setCurrentText(str(getattr(obj, "GeometryMode", "Rectangle")))
            self.width.setValue(float(obj.Width)); self.height.setValue(float(obj.Height)); self.allowance.setValue(float(obj.SeamAllowance)); self.grain.setValue(float(obj.GrainlineAngle))
            ...
            self.mode.setCurrentText("Rectangle"); self._original = None
        self._mode_changed(self.mode.currentText())
        self.form.show()

    def _mode_changed(self, mode):
        custom = mode == "Custom"; self.width.setEnabled(not custom); self.height.setEnabled(not custom)
    def _validate(self):
...
class PatternDraftingTaskPanel:
    ...
    def __init__(self, obj):
        ...
        self.move_point, self.add_point, self.remove_point = move_point, add_point, remove_point; self.seam_allowance_preview = seam_allowance_preview; self.selected = 0
        self.form = QtWidgets.QWidget(); self.form.setWindowTitle("Cloth Pattern — 2D Drafting"); outer = QtWidgets.QVBoxLayout(self.form)
        self.canvas = QtWidgets.QGraphicsView(); self.scene = QtWidgets.QGraphicsScene(self.canvas); self.canvas.setScene(self.scene); self.canvas.setMinimumSize(500, 360); outer.addWidget(self.canvas); self.scene.selectionChanged.connect(self._selection_changed)
        ...
        add_button = QtWidgets.QPushButton("Add point"); add_button.clicked.connect(self.add_selected_point); controls.addWidget(add_button)
        remove_button = QtWidgets.QPushButton("Remove point"); remove_button.clicked.connect(self.remove_selected_point); controls.addWidget(remove_button)
