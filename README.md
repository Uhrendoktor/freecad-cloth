...
The canonical FreeCAD/Xvfb workflow publishes the validated GUI states to a stable `docs/screenshots` branch. The avatar audit renders the production mannequin from a complete 360° camera path so the silhouette can be inspected continuously rather than from six disconnected stills. The avatar turntable below is pinned to the stable screenshot publication generated by the canonical workflow.
