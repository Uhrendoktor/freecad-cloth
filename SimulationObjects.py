...
class SimulationProxy:
    ...
    def __init__(self):self.system=None;self.left_indices=None;self.right_indices=None;self.last_steps=0
    def execute(self,obj):
        if self.system is None or int(obj.Steps)<self.last_steps:self._build(obj)
        steps=int(obj.Steps)
        if steps>self.last_steps:
            for _ in range(steps-self.last_steps):self.system.step(float(obj.TimeStep),int(obj.Iterations),(float(obj.GravityX),float(obj.GravityY),float(obj.GravityZ)),(float(obj.CollisionX),float(obj.CollisionY),float(obj.CollisionZ),float(obj.CollisionRadius)))
    ...
    def _build(self,obj):
        ...
        nx,ny=8,5;left=ClothSystem.grid(100.,60.,nx,ny,origin=(-100.,-30.,90.));right=ClothSystem.grid(100.,60.,nx,ny,origin=(0.,-30.,90.));offset=len(left.particles);particles=left.particles+right.particles;constraints=list(left.constraints)+[type(c)(c.a+offset,c.b+offset,c.rest,c.compliance) for c in right.constraints];self.system=ClothSystem(particles,constraints);self.left_indices=tuple(range(offset));self.right_indices=tuple(range(offset,offset*2));default_seams=[(j*nx+nx-1,offset+j*nx) for j in range(ny)];self.system.add_stitches(_parse_pair_list(obj.SeamSelection,len(particles)) or tuple(default_seams));default_pins=(0,nx-1,offset,offset+nx-1);self.system.pin(_parse_int_list(obj.PinSelection,len(particles)) or default_pins);self.last_steps=0
...
def create_simulation_scene(doc):
    ...
    proxy=SimulationProxy();scene.Proxy=proxy;a=_mesh_object(doc,"DrapePanelA","Drape Panel A");b=_mesh_object(doc,"DrapePanelB","Drape Panel B");scene.ClothPieces=[a,b];proxy._build(scene);_write_grid_mesh(a,proxy.system,proxy.left_indices,8,5);_write_grid_mesh(b,proxy.system,proxy.right_indices,8,5);avatar=doc.addObject("Part::Feature","AvatarCollision");avatar.Label="Avatar Collision Proxy";avatar.Shape=Part.makeSphere(38.,FreeCAD.Vector(0,0,0));avatar.addProperty("App::PropertyString","CollisionType","Simulation").CollisionType="SphereProxy";scene.AvatarProxy=avatar;return scene
