...
def _attach_to_simulation(doc, target):
    """Attach the persistent target without replacing it with AvatarProxy."""
...
def _target_is_ready():
    try:
        ...
        target = _target(App.ActiveDocument) if App.ActiveDocument else None
        return target is not None and not target_status(target)["stale"]
