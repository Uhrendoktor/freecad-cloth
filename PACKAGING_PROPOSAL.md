...
`Mod` directory. The existing repository follows that classic layout with
`Init.py` and `InitGui.py` at the workbench root. A `pyproject.toml` file is
...
match for pip/standard-Python packaging. It moves the runtime code under a
`freecad/<workbench>/` namespace and keeps `init.py`/`init_gui.py` as the
FreeCAD entry points. That migration should be a separate, deliberately tested
change rather than being hidden inside this packaging proposal.
...
- `requires-python = ">=3.8"` preserves compatibility with the Python baseline
  documented for FreeCAD 1.0. Newer FreeCAD development may require newer
  Python, but that should be raised only when this workbench's supported
  runtime matrix is intentionally raised.
...
exercise a real FreeCAD smoke test, and only then enable setuptools package
discovery for an actual wheel installation.
