...
def _selected_pattern_edges(allow_many=False):
    ...
    if (not allow_many and len(edges)!=2) or (allow_many and len(edges)<2):
        count="at least two" if allow_many else "exactly two"; raise ValueError("select %s edges on pattern pieces"%count)
...
def _has_two_selected_pattern_edges():
    try: _selected_pattern_edges(); return True
    except (ImportError,ValueError): return False
def _has_mn_selection():
    try: _selected_pattern_edges(allow_many=True); return True
    except (ImportError,ValueError): return False
...
def create_mn_sewing_from_selection():
    ...
    doc=App.ActiveDocument
    if doc is None: raise ValueError("open a document before creating sewing")
    selected=_selected_pattern_edges(allow_many=True); pieces=_pieces_by_id(doc); piece_ids=[]
    ...
    for member in side_a+side_b: lengths[(member.piece_id,member.edge)]=_edge_length(pieces[member.piece_id],member.edge)
    existing_ids={str(getattr(obj,"RelationshipId","")) for obj in doc.Objects if getattr(obj,"SewingType","")=="SewingNetwork"}; index=len(existing_ids)+1; relationship_id="sewing-%d"%index
    while relationship_id in existing_ids: index+=1; relationship_id="sewing-%d"%index
    models=build_mn_seams(relationship_id,side_a,side_b,lengths); seam_objects=[add_seam(doc,model) for model in models]
    network=add_sewing_network(doc,seam_objects,relationship_id,"SewingNetwork%d"%index); doc.recompute(); return network
...
def create_sewing_operation():
    ...
    try: a,b=pieces[str(seam.PieceA)],pieces[str(seam.PieceB)]
    except KeyError: raise ValueError("the seam references pattern pieces that are not in the active document")
    n=len([o for o in doc.Objects if getattr(o,"SewingType","")=="SewingOperation"])+1; obj=add_sewing_operation(doc,seam,a,b,"SewingOperation%d"%n); doc.recompute(); return obj
...
def reverse_selected_seam():
    import FreeCAD as App
    doc=App.ActiveDocument
    if doc is None: raise ValueError("open a document before changing seam orientation")
    seam=_selected_seam(doc); seam.ReversedB=not bool(seam.ReversedB); doc.recompute(); return seam

def toggle_selected_alignment():
    import FreeCAD as App
    doc=App.ActiveDocument
    if doc is None: raise ValueError("open a document before changing seam alignment")
    seam=_selected_seam(doc); seam.Alignment="uniform" if str(seam.Alignment)=="endpoints" else "endpoints"; doc.recompute(); return seam

def validate_seams():
    import FreeCAD as App
    doc=App.ActiveDocument
    if doc is None: return []
    ops=[o for o in doc.Objects if getattr(o,"SewingType","")=="SewingOperation"]; networks=[o for o in doc.Objects if getattr(o,"SewingType","")=="SewingNetwork"]
    for obj in ops+networks: obj.Proxy.execute(obj)
    doc.recompute(); return ([(o.Name,o.Status,float(o.LengthDifference)) for o in ops]+[(o.Name,o.Status,float(o.LengthDifference)) for o in networks])

def repair_selected_seam():
    import FreeCAD as App
    doc=App.ActiveDocument
    if doc is None: raise ValueError("open a document before repairing a seam")
    seam=_selected_seam(doc)
    from SewingGui import correspondence_report, repair_correspondence_settings
    length_a=float(getattr(seam,"LengthA",0.0)); length_b=float(getattr(seam,"LengthB",0.0))
    if length_a <= 0.0 or length_b <= 0.0:
        pieces=_pieces_by_id(doc)
        try:
            from SewingObjects import _edge_length
            length_a=_edge_length(pieces[str(seam.PieceA)],int(seam.EdgeA)); length_b=_edge_length(pieces[str(seam.PieceB)],int(seam.EdgeB))
        except Exception as exc: raise ValueError("cannot determine seam lengths for repair: %s"%exc)
    report=correspondence_report(seam,length_a,length_b,0.05)
    message=repair_correspondence_settings(seam,report); doc.recompute(); return message
...
COMMANDS=["ClothSewing_CreateSeam","ClothSewing_CreateMNSewing","ClothSewing_CreateOperation","ClothSewing_EditOperation","ClothSewing_ReverseSeam","ClothSewing_ToggleAlignment","ClothSewing_Validate","ClothSewing_RepairSeam","ClothSewing_Show2D"]
_COMMAND_HANDLERS={
    "ClothSewing_CreateSeam":create_seam_from_selection,"ClothSewing_CreateMNSewing":create_mn_sewing_from_selection,"ClothSewing_CreateOperation":create_sewing_operation,"ClothSewing_EditOperation":edit_sewing_operation,"ClothSewing_ReverseSeam":reverse_selected_seam,"ClothSewing_ToggleAlignment":toggle_selected_alignment,"ClothSewing_Validate":validate_seams,"ClothSewing_RepairSeam":repair_selected_seam,"ClothSewing_Show2D":show_sewing_2d,
}
_MENU_TEXT={"ClothSewing_CreateSeam":"Create Seam","ClothSewing_CreateMNSewing":"Create M:N Sewing","ClothSewing_CreateOperation":"Create Sewing Operation","ClothSewing_EditOperation":"Edit Sewing Operation","ClothSewing_ReverseSeam":"Reverse Seam","ClothSewing_ToggleAlignment":"Toggle Seam Alignment","ClothSewing_Validate":"Validate Sewing","ClothSewing_RepairSeam":"Repair Seam","ClothSewing_Show2D":"Show Sewing 2D"}
_TOOLTIPS={"ClothSewing_CreateSeam":"Create a persistent seam from two selected pattern edges","ClothSewing_CreateMNSewing":"Create a deterministic 1:N, M:1, or M:N sewing relationship from selected edges","ClothSewing_CreateOperation":"Create a sewing operation from the selected seam","ClothSewing_EditOperation":"Edit seam alignment, orientation, tolerance, and stitch samples","ClothSewing_ReverseSeam":"Reverse the B-side stitch correspondence","ClothSewing_ToggleAlignment":"Toggle endpoint and uniform seam correspondence","ClothSewing_Validate":"Validate sewing operations and report seam length mismatches","ClothSewing_RepairSeam":"Repair reversible or invalid-range seam correspondence without hiding length mismatch","ClothSewing_Show2D":"Show pattern, seam, and stitch correspondence in top view"}
...
class _SewingCommand:
    ...
    def GetResources(self):
        ...
        if self.pixmap: resources["Pixmap"]=self.pixmap
        return resources
try:
    import FreeCADGui as Gui
    ...
    for name,function in _COMMAND_HANDLERS.items():
        ...
        if not icon_path.is_file() and name=="ClothSewing_RepairSeam": icon_path=_ICON_DIR/"ClothSewing_Validate.svg"
