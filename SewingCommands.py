...
def create_sewing_operation():
    """Create a persistent sewing operation from a selected seam."""
    ...
    try:
        piece_a = pieces[str(seam.PieceA)]
        piece_b = pieces[str(seam.PieceB)]
    except KeyError:
    ...
    obj = add_sewing_operation(doc, seam, piece_a, piece_b, "SewingOperation%d" % index)
    doc.recompute()
    return obj


def edit_sewing_operation():
    """Open the task panel for the selected sewing operation."""
    import FreeCADGui as Gui
...
def validate_seams():
    """Recompute all sewing operations and return their validation status."""
    import FreeCAD as App
    ...
    doc.recompute()
    return [(obj.Name, obj.Status, float(obj.LengthDifference)) for obj in operations]


def show_sewing_2d():
    """Switch the active document to a top-down view for seam inspection."""
    import FreeCADGui as Gui
...
class _FunctionCommand:
    ...
    def GetResources(self):
        return {
            ...
            "ToolTip": self.function.__doc__ or "Cloth sewing command",
        }
...
try:
    ...
    if hasattr(Gui, "addCommand"):
        Gui.addCommand("ClothSewing_CreateOperation", _FunctionCommand(create_sewing_operation))
        Gui.addCommand("ClothSewing_EditOperation", _FunctionCommand(edit_sewing_operation))
        Gui.addCommand("ClothSewing_Validate", _FunctionCommand(validate_seams))
        Gui.addCommand("ClothSewing_Show2D", _FunctionCommand(show_sewing_2d))
