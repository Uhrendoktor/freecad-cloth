...
def validate_seams():
    ...
    graph=validate_sewing_graph(doc)
    return {"status":graph["status"],"message":graph["message"],"operations":[(o.Name,o.Status,float(o.LengthDifference)) for o in ops],"networks":[(o.Name,o.Status,float(o.LengthDifference)) for o in networks],"isolated":graph["isolated"]}

def inspect_sewing_graph():
    import FreeCAD as App
    doc=App.ActiveDocument
...
def repair_selected_seam():
    ...
    if length_a<=0.0 or length_b<=0.0:
        ...
        try:
            ...
            length_a=_seam_length(pieces[str(seam.PieceA)],seam,"A"); length_b=_seam_length(pieces[str(seam.PieceB)],seam,"B")
        except Exception as exc: raise ValueError("cannot determine seam lengths for repair: %s"%exc)
    report=correspondence_report(seam,length_a,length_b,0.05); message=repair_correspondence_settings(seam,report); doc.recompute(); return message
...
try:
    ...
    for name,function in _COMMAND_HANDLERS.items():
        ...
        if not icon_path.is_file(): icon_path=_ICON_DIR/"ClothSewing_Validate.svg"
