...
def _install_drape_target_recompute_guard():
    ...
    SimulationObjects owns the solver implementation, but a FreeCAD document
    recompute must never raise merely because an authored collision target needs
    refreshing. The proxy is therefore wrapped at workbench import time: stale
    target state is surfaced through the existing status contract and the old
    solver state is left untouched until an explicit Refresh rebuilds it.
    """
    try:
