...
def refresh_drape_target():
    ...
    if target is None:
        raise RuntimeError("no DrapeTarget in active document")
    refresh(target)
    doc.recompute()
    return target
...
def simulation_status():
    try:
        ...
        return {"state": "unavailable", "message": str(exc), "steps": 0, "particles": 0, "time": 0.0,
                "target_state": "invalid", "target_lifecycle_state": "INVALID", "target_message": "No drape target selected", "target_stale": True,
    ...
    try:
        ...
        target_info = {"state": "invalid", "lifecycle_state": "INVALID", "message": "Cannot inspect drape target: %s" % exc, "stale": True,
    ...
    return {
        ...
        "message": "Cloth Simulation ready" if ready else "Cloth Simulation is blocked until the DrapeTarget is ready",
        ...
        "target_lifecycle_state": target_info["lifecycle_state"],
...
def _has_ready_simulation():
    try:
        ...
        from DrapeTarget import target_status
        return target_status(_find_drape_target(doc))["lifecycle_state"] == "READY_FOR_SIMULATION"
    except (ImportError, AttributeError, TypeError, ValueError):
        return False


COMMANDS = [
    "ClothSimulation_Create", "ClothSimulation_CreateDrape", "ClothSimulation_Edit",
    "ClothSimulation_RefreshDrapeTarget", "ClothSimulation_Step", "ClothSimulation_Run", "ClothSimulation_Reset",
]
try:
