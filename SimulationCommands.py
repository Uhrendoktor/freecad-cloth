...
def create_simulation():
    ...
    install_quality_mesh_patch(); doc = App.ActiveDocument or App.newDocument("ClothSimulation")
...
def _target_gate(doc):
    ...
    if status["stale"]: raise RuntimeError(status["message"])
...
def simulate_selected(steps=None):
    doc, scene = _require_simulation(); _target_gate(doc)
...
def simulation_status():
    ...
    try:
        ...
        target_info = {"state": "invalid", "message": "Cannot inspect drape target: %s" % exc, "stale": True, "reason": "target inspection failed"}
    return {"state": "ready" if finite and not target_info["stale"] else "invalid/non-finite", "message": "Cloth Simulation ready" if finite and not target_info["stale"] else target_info["message"], "steps": int(getattr(scene, "Steps", 0)), "particles": int(getattr(scene, "ParticleCount", 0)), "time": float(getattr(scene, "SimulatedTime", 0.0)), "target_state": target_info["state"], "target_message": target_info["message"], "target_stale": bool(target_info["stale"]), "target_reason": target_info["reason"]}
