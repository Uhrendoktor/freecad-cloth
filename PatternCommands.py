...
def _freecad_ci_progress(message):
    directory = os.environ.get("CLOTH_E2E_DIR")
    if not directory:
        return
    try:
        os.makedirs(directory, exist_ok=True)
        with open(os.path.join(directory, "freecad-import-progress.log"), "a", encoding="utf-8") as handle:
            handle.write(message + "\n")
            handle.flush()
    except Exception:
        pass

_freecad_ci_progress("PatternCommands: module-start")
...
try:
    ...
    _freecad_ci_progress("PatternCommands: imported-FreeCADGui")
except ImportError:
    Gui = None
...
def edit_pattern_piece():
    import FreeCADGui as Gui
    selection = Gui.Selection.getSelection(); obj = next((o for o in selection if getattr(o, "PatternType", "") == "PatternPiece"), None)
    if obj is None: raise ValueError("select a pattern piece before editing it")
    from PatternGui import show_pattern_piece_task; show_pattern_piece_task(obj)
...
def create_pattern_piece_task():
    from PatternGui import show_pattern_piece_task; show_pattern_piece_task()
...
def show_pattern_2d():
    from PatternGui import show_pattern_view; show_pattern_view()
...
def add_seam():
    ...
    base=f"{piece_a.PieceId}-e{edge_a}-{piece_b.PieceId}-e{edge_b}"; existing={getattr(o,"SeamId","") for o in doc.Objects}; seam_id,suffix=base,2
...
_freecad_ci_progress("PatternCommands: module-done")
